...
function DemographicBreakdownChart({
  breakdown,
}: {
  breakdown: DemographicBiasBreakdown[]
}) {
          ...
          <div className='flex items-center justify-between'>
            ...
            <span
              className={cn(
                ...
                concernColors[demo.concernLevel],
                'bg-opacity-20',
              )}
            >
              {demo.concernLevel}
    ...
    <div className='space-y-2'>
      {datasets.items.map((dataset) => (
        <button
          ...
          onClick={() =>
            onSelect(
              dataset.datasetId === selectedId ? null : dataset.datasetId,
            )
          }
          className={cn(
...
function AuditDetailPanel({
  ...
  isLoading: boolean
  onAction: (
    action: 'approve' | 'quarantine' | 'reject' | 'request_reaudit',
    reason?: string,
  ) => void
            ...
            <CardContent>
              <div className='flex items-center justify-between'>
                ...
                <span
                  className={cn(
                    'text-sm',
                    audit.passesThreshold ? 'text-green-400' : 'text-red-400',
                  )}
                >
                  {audit.passesThreshold
                    ? '✓ Passes threshold'
                    : '✗ Exceeds threshold'}
                </span>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <DemographicBreakdownChart
                breakdown={audit.demographicBreakdown}
              />
            </CardContent>
          </Card>
