...
export interface AIModelConfig {
  modelId: string;
  modelType: 'anomaly_detection' | 'classification' | 'clustering' | 'prediction';
  modelPath: string;
  inputShape: number[];
  outputShape: number[];
  threshold: number;
...
export interface HuntPattern {
  ...
  name: string;
  description: string;
  patternType: 'network' | 'endpoint' | 'user_behavior' | 'malware' | 'lateral_movement' | 'custom';
  query: string;
    ...
    mongodb: {
      ...
      collections: [
        ...
        },
        {
          name: 'indicators',
          indexes: [
            { fields: { value: 1, indicatorType: 1 }, options: { unique: true } },
    ...
    escalationRules: [
      {
        ...
        escalationLevel: 1,
        notificationRecipients: ['security-team@company.com', 'ciso@company.com'],
        autoEscalate: true
      }
    ],
    ...
    huntPatterns: [
      {
        ...
        patternType: 'network',
        query: 'SELECT * FROM network_logs WHERE bytes_transferred > 1000000 AND destination_port IN (22, 23, 135, 139, 445, 1433, 3389)',
        severity: 'high',
        confidence: 0.8,
        indicators: ['suspicious_connection', 'port_scanning', 'data_exfiltration'],
        conditions: [
          { field: 'bytes_transferred', operator: 'greater_than', value: 1000000, weight: 0.7 },
          { field: 'destination_port', operator: 'in', value: [22, 23, 135, 139, 445, 1433, 3389], weight: 0.3 }
        ],
        actions: [
          {
    ...
    feedSources: [
      {
        ...
        filters: { minConfidence: 0.5, severity: ['medium', 'high', 'critical'] },
        enabled: true
      },
      {
...
export class ThreatIntelligenceConfigManager {
  ...
  constructor(config: ThreatIntelligenceConfig = DEFAULT_THREAT_INTELLIGENCE_CONFIG) {
    this.config = config;
  }
  ...
  private async initializeMongoDB(): Promise<void> {
    try {
      this.mongoClient = new MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017/threat_intelligence');
  ...
  async addRegionConfig(regionConfig: RegionConfig): Promise<void> {
    try {
      ...
      const existingRegion = this.config.regions.find(r => r.regionId === regionConfig.regionId);
      if (existingRegion) {
