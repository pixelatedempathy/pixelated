...
export class MemoryMonitor {
  ...
  getResults(): { initial: number; peak: number; final: number; average: number } {
...
export class PerformanceBenchmarkRunner {
  ...
  private async runScenarioBenchmark(scenario: any, concurrentUsers: number): Promise<PerformanceResults> {
  ...
  private async makeTestRequest(scenario: any): Promise<{ success: boolean; responseTime: number }> {
...
if (typeof require !== 'undefined' && require.main === module) {
  ...
  runner.runFullBenchmark()
    ...
    .then(memoryResults => {
      ...
      console.log(`   Memory Increase: ${(memoryResults.memoryIncrease / 1024 / 1024).toFixed(2)} MB`)
      console.log(`   Leak Detected: ${memoryResults.leakDetected ? 'YES' : 'NO'}`)
