...
import type { PerformanceMetrics, OptimizationRecommendation } from '@/types/monitoring'
...
class PerformanceMonitor {
  ...
  private async measureErrorRate(): Promise<number> {
    ...
    const totalRequests = recentMetrics.reduce((sum, m) => sum + (m.requestCount || 1), 0)
  ...
  private analyzePerformance(): void {
    ...
    const recommendations = this.generateOptimizationRecommendations(recentAvg, olderAvg)
  ...
  private generateOptimizationRecommendations(
    ...
    if (recent.responseTime > this.config.alertThresholds.responseTime) {
      const degradation = ((recent.responseTime - older.responseTime) / older.responseTime) * 100
      if (degradation > 10) {
        recommendations.push(`Response time degraded by ${degradation.toFixed(1)}% - consider caching optimization`)
      }
    }

    // Memory usage analysis
    if (recent.memoryUsage > this.config.alertThresholds.memoryUsage) {
      recommendations.push('Memory usage above threshold - consider garbage collection optimization')
    }
  ...
  private calculateExpectedImprovement(recommendations: string[]): Record<string, number> {
  ...
  private checkAlerts(): void {
    ...
    if (latest.responseTime > this.config.alertThresholds.responseTime) {
      this.createAlert('warning', 'responseTime', latest.responseTime, this.config.alertThresholds.responseTime,
    ...
    if (latest.memoryUsage > this.config.alertThresholds.memoryUsage) {
      this.createAlert('critical', 'memoryUsage', latest.memoryUsage, this.config.alertThresholds.memoryUsage,
    ...
    if (latest.cpuUsage > this.config.alertThresholds.cpuUsage) {
      this.createAlert('warning', 'cpuUsage', latest.cpuUsage, this.config.alertThresholds.cpuUsage,
    ...
    if (latest.errorRate > this.config.alertThresholds.errorRate) {
      this.createAlert('critical', 'errorRate', latest.errorRate, this.config.alertThresholds.errorRate,
   ...
   */
  onAlert(type: 'warning' | 'critical' | 'info', callback: (alert: PerformanceAlert) => void): () => void {
  ...
  getPerformanceTrends(hours: number = 24): {
    ...
    return {
      responseTime: calculateTrend(firstAvg.responseTime, secondAvg.responseTime),
  ...
  getSystemHealth(): SystemHealth {
    ...
    const components: Record<string, any> = {
      api: {
        status: current.responseTime < this.config.alertThresholds.responseTime ? 'healthy' : 'warning',
        responseTime: current.responseTime,
        errorRate: current.errorRate,
        lastCheck: current.timestamp,
      },
      database: {
        status: current.memoryUsage < this.config.alertThresholds.memoryUsage ? 'healthy' : 'warning',
      ...
      realtime: {
        status: current.cpuUsage < this.config.alertThresholds.cpuUsage ? 'healthy' : 'critical',
    ...
    if (current.memoryUsage > this.config.optimizationTargets.memoryUsage) {
      recommendations.push('Reduce memory usage through code optimization and garbage collection')
    }
  ...
  getPerformanceAnalytics(): {
    ...
    const healthyTime = recentMetrics.filter(m =>
  ...
  private generateOverallRecommendations(): string[] {
    ...
    if (trends.responseTime.trend === 'up') {
      recommendations.push('Response time is increasing - investigate recent changes')
    }
  ...
  exportPerformanceData(format: 'json' | 'csv' = 'json'): string {
    ...
    if (format === 'json') {
      ...
      const headers = ['timestamp', 'responseTime', 'memoryUsage', 'cpuUsage', 'errorRate', 'throughput']
