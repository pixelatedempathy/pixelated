...
describe('Auth0 RBAC Service', () => {
  ...
  describe('Role Definitions', () => {
    it('should have correct role definitions', () => {
      ...
      expect(Object.keys(auth0RbacService.AUTH0_ROLE_DEFINITIONS)).toHaveLength(
        6,
      )
  ...
  describe('Permission Definitions', () => {
    it('should have correct permission definitions', () => {
      ...
      const readPatientsPerm =
        auth0RbacService.AUTH0_PERMISSION_DEFINITIONS['read:patients']
      expect(readPatientsPerm).toBeDefined()
      expect(readPatientsPerm.name).toBe('read:patients')
      expect(readPatientsPerm.description).toBe(
        'View patient medical records and profiles',
      )
      ...
      const writeUsersPerm =
        auth0RbacService.AUTH0_PERMISSION_DEFINITIONS['write:users']
  ...
  describe('assignRoleToUser', () => {
    ...
    it('should throw error when role is not found', async () => {
      ...
      await expect(
        auth0RbacService.assignRoleToUser('auth0|user123', 'nonexistent-role'),
    ...
    it('should handle assignment errors', async () => {
      ...
      mockManagementClient.assignRolestoUser.mockRejectedValue(
        new Error('Assignment failed'),
      )

      await expect(
        auth0RbacService.assignRoleToUser('auth0|user123', 'therapist'),
  ...
  describe('removeRoleFromUser', () => {
    ...
    it('should throw error when role is not found', async () => {
      ...
      await expect(
        auth0RbacService.removeRoleFromUser(
          'auth0|user123',
          'nonexistent-role',
        ),
      ).rejects.toThrow('Role nonexistent-role not found')
  ...
  describe('getUserRoles', () => {
    it('should successfully retrieve user roles', async () => {
      ...
      expect(mockManagementClient.getUserRoles).toHaveBeenCalledWith({
        id: 'auth0|user123',
      })
    })
  ...
  describe('roleHasPermission', () => {
    it('should return true for admin role with any permission', () => {
      const hasPermission = auth0RbacService.roleHasPermission(
        'admin',
        'read:patients',
      )
      expect(hasPermission).toBe(true)
  ...
  describe('hasRequiredRole', () => {
    ...
    it('should return false when user role has lower hierarchy level', () => {
      const hasRequired = auth0RbacService.hasRequiredRole(
        'patient',
        'therapist',
      )
  ...
  describe('requiresAudit', () => {
    ...
    it('should return false for permissions that do not require audit', () => {
      ...
      const requiresAudit = auth0RbacService.requiresAudit(
  ...
  describe('canAssignRole', () => {
    ...
    it('should return false for non-existent roles', () => {
      const canAssign = auth0RbacService.canAssignRole(
        'nonexistent' as any,
        'therapist',
      )
      expect(canAssign).toBe(false)
  ...
  describe('validateRoleTransition', () => {
    ...
    it('should throw error for same role transition', () => {
      expect(() =>
        auth0RbacService.validateRoleTransition('therapist', 'therapist'),
