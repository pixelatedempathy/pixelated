...
class TaskAssignmentService:
    ...
    def _filter_eligible_agents(self, task: Task, agents: List[Agent]) -> List[Agent]:
        ...
        for agent in agents:
            ...
            if task.memory_requirements_mb and agent.memory_limit_mb:
                if task.memory_requirements_mb > agent.memory_limit_mb:
                    continue
            
            # Check task type compatibility
            if task.task_type and agent.supported_task_types:
    ...
    def _score_priority_based(self, task: Task, agent: Agent) -> AssignmentScore:
        ...
        Returns:
            Assignment score
        """
        # Check if agent has priority handling capability
        priority_capability = None
        for cap in agent.capabilities:
            if cap.name == "priority_handling":
                priority_capability = cap
                break
        ...
        if not priority_capability:
            ...
            if capability_level >= priority_level:
                ...
                score = capability_level / priority_level
