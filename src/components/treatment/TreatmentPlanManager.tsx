...
const TreatmentPlanManager: React.FC<TreatmentPlanManagerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'goals' | 'progress' | 'notes'>('overview')
  ...
  const defaultPlan: TreatmentPlan = {
    ...
    duration: 12,
    status: 'active',
    notes: 'Initial assessment shows moderate anxiety and depression symptoms. Client is motivated for change.',
    goals: [
      {
        id: 'goal-1',
        title: 'Reduce Anxiety Symptoms',
        description: 'Learn and practice anxiety management techniques to reduce daily anxiety levels',
        ...
        category: 'emotional',
        milestones: [
          { id: 'm1', title: 'Learn breathing techniques', completed: true, completedDate: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) },
          { id: 'm2', title: 'Practice daily meditation', completed: true, completedDate: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
      ...
      },
      {
        ...
        description: 'Establish healthy sleep patterns and improve sleep duration and quality',
        ...
        category: 'physical',
        milestones: [
          { id: 'm5', title: 'Create bedtime routine', completed: true, completedDate: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000) },
      ...
      },
      {
        ...
        description: 'Build and maintain meaningful relationships and social support networks',
  ...
  const overallProgress = Math.round(
    currentPlan.goals.reduce((sum, goal) => sum + goal.progress, 0) / currentPlan.goals.length
  )
      ...
      <div className="border-b border-gray-200 mb-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-gray-600">Therapist: {currentPlan.therapistName}</p>
          </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <div className="text-blue-600 text-lg font-semibold">Duration</div>
              <div className="text-2xl font-bold">{currentPlan.duration} weeks</div>
            </div>
            <div className="bg-green-50 p-4 rounded-lg">
              <div className="text-green-600 text-lg font-semibold">Active Goals</div>
              <div className="text-2xl font-bold">{currentPlan.goals.length}</div>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg">
              <div className="text-purple-600 text-lg font-semibold">Status</div>
              <div className="text-2xl font-bold capitalize">{currentPlan.status}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              {['behavioral', 'cognitive', 'emotional', 'social', 'physical'].map((category) => {
                const count = currentPlan.goals.filter(g => g.category === category).length
                return (
                  <div key={category} className="text-center p-3 bg-gray-50 rounded-lg">
                    <div className="text-2xl mb-1">{getCategoryIcon(category as TreatmentGoal['category'])}</div>
        ...
        <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-3">
              ...
              <div className="flex gap-4">
                <select
                  ...
                  onChange={(e) => setNewGoal({ ...newGoal, priority: e.target.value as TreatmentGoal['priority'] })}
                ...
                <select
                  ...
                  onChange={(e) => setNewGoal({ ...newGoal, category: e.target.value as TreatmentGoal['category'] })}
                  className="p-2 border rounded"
                >
                ...
                <input
                  type="date"
                  value={newGoal.targetDate ? format(newGoal.targetDate, 'yyyy-MM-dd') : format(addDays(new Date(), 30), 'yyyy-MM-dd')}
                  onChange={(e) => setNewGoal({ ...newGoal, targetDate: new Date(e.target.value) })}
                  className="p-2 border rounded"
                />
              </div>
              <div className="flex gap-2">
                <button onClick={addNewGoal} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
                  Add Goal
                </button>
                <button onClick={() => setShowAddGoal(false)} className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">
                  Cancel
                </button>
              </div>
            </div>
          ...
          <div className="space-y-4">
            {currentPlan.goals.map((goal) => (
              <div key={goal.id} className="border rounded-lg p-4 bg-white shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-500">
                      ...
                      ({differenceInDays(goal.targetDate, new Date())} days remaining)
                    </div>
                  </div>
      ...
      {activeTab === 'notes' && (
        <div className="space-y-4">
          ...
          <textarea
            ...
            onChange={(e) => {
              if (!readOnly) {
                const updatedPlan = { ...currentPlan, notes: e.target.value, lastModified: new Date() }
                setCurrentPlan(updatedPlan)
                onSave?.(updatedPlan)
              }
            }}
          ...
          />
          <div className="text-sm text-gray-500">
            Last modified: {format(currentPlan.lastModified, 'MMM dd, yyyy at h:mm a')}
          </div>
        </div>
      )}
    </div>
