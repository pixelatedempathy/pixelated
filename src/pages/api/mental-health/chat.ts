...
function determineStressLevel(
  ...
  if (
    STRESS_INDICATORS.high.some((indicator) => lowerMessage.includes(indicator))
  ) {
    return 'high'
  }
  if (
    STRESS_INDICATORS.moderate.some((indicator) =>
  ...
  if (
    STRESS_INDICATORS.low.some((indicator) => lowerMessage.includes(indicator))
  ) {
    return 'low'
  }
...
function generateResponse(
  ...
  riskAssessment?: ChatResponse['riskAssessment'],
  ...
  if (riskAssessment?.crisisLevel === 'imminent') {
    responseMessage =
      "I'm very concerned about what you've shared. Your safety is the most important thing right now. Please reach out for immediate help by calling 988 (National Suicide Prevention Lifeline) or going to your nearest emergency room. You don't have to go through this alone."
    responseType = 'crisis_intervention'
  } else if (riskAssessment?.crisisLevel === 'high') {
    responseMessage =
      "Thank you for sharing something so difficult with me. I'm concerned about how you're feeling. Please consider reaching out to a mental health professional or calling 988 for support. In the meantime, let's work on some coping strategies together."
    responseType = 'crisis_intervention'
  } else if (analysis.stressLevel === 'high') {
    responseMessage =
      "It sounds like you're going through a really challenging time right now. That level of stress can feel overwhelming. Let's focus on some immediate coping strategies that might help you feel more grounded."
    responseType = 'coping_strategy'
  } else if (analysis.stressLevel === 'moderate') {
    responseMessage =
      "I hear that you're dealing with some stress and difficult feelings. It's completely normal to have ups and downs. Would you like to explore what's contributing to these feelings or work on some coping strategies?"
    responseType = 'supportive'
  } else {
    responseMessage =
      "Thank you for sharing with me. It sounds like you're managing things relatively well. Is there anything specific you'd like to talk about or work on today?"
    responseType = 'supportive'
  }
...
export const POST = async ({ request }: APIContext) => {
  ...
  try {
    ...
    const concernSeverity = Math.max(
      1,
      Math.min(
        10,
        5 +
    ...
    const riskAssessment = options.includeRiskAssessment
      ? assessRisk(body.message, body.userContext)
      : undefined
    ...
    if (riskAssessment?.immediateAction) {
      flags.push('IMMEDIATE_ACTION_REQUIRED')
    }
    if (
      riskAssessment?.crisisLevel === 'high' ||
      riskAssessment?.crisisLevel === 'imminent'
    ...
    const chatResponse: ChatResponse = {
      ...
      followUp: {
        ...
        scheduledReminders:
          riskAssessment?.crisisLevel !== 'none'
            ? ['Daily check-in reminder', 'Coping strategy practice']
            : [],
      },
      metadata: {
