...
try {
  ...
  for (const [header, expectedValue] of Object.entries(requiredHeaders)) {
    if (header === 'Content-Security-Policy') {
      ...
      const headerPattern = new RegExp(`response\\.headers\\.set\\([^)]*${escapedHeader}[^)]*['"]${escapedValue}['"]`, 'i');
    ...
    } // <-- close the 'else' for non-special headers
  } // <-- close the 'for' loop over requiredHeaders
