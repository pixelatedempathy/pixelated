...
const RubiksCubeModel = forwardRef<THREE.Group, RubiksCubeModelProps>(
  (props, ref) => {
    ...
    const startRotation = useCallback((move: Move): void => {
      ...
      const animate = () => {
        ...
        const rotationAmount = isFinished
          ? targetRotation - (currentRotation - rotationSpeed)
          : rotationSpeed
        const stepRotationMatrix = getRotationMatrix(
          move.axis,
          rotationAmount * move.direction,
        )
        ...
        setCubes((prevCubes) =>
          prevCubes.map((cube) => {
            ...
            if (Math.round(posValue) === move.layer) {
              const newRotationMatrix = reusableMatrix4
                .clone()
    ...
    useEffect(() => {
      finalizeRotationRef.current = (move: Move) => {
        const finalRotationMatrix = getRotationMatrix(
          move.axis,
          (Math.PI / 2) * move.direction,
        )
        setCubes((prevCubes) => {
          const newCubes = prevCubes.map((cube) => {
            ...
            if (Math.round(posValue) === move.layer) {
              const newPosition = cube.position
                .clone()
                .applyMatrix4(finalRotationMatrix)
              return {
                ...cube,
