...
export const extractBehavioralFeatures = (data: any[]) => {
  const loginEvents = data.filter(
    (e) => e.action === 'login' || e.eventType === 'login',
  )
  const logoutEvents = data.filter(
    (e) => e.action === 'logout' || e.eventType === 'logout',
  )
  const dataAccessEvents = data.filter(
    (e) => e.action === 'data_access' || e.eventType === 'data_access',
  )
...
export const detectPatternChanges = (historical: any[], current: any[]) => {
  ...
  const significant =
    Math.abs(currFeatures.loginFrequency - histFeatures.loginFrequency) > 2

  return {
    loginChange: {
      significant,
      delta: currFeatures.loginFrequency - histFeatures.loginFrequency,
    },
    dataAccessChange: {
      delta:
        currFeatures.dataAccessFrequency - histFeatures.dataAccessFrequency,
    },
    newPatterns: {},
  }
}
