    ...
    return new Promise((resolve, reject) => {
        server.listen(port, 'localhost', async () => {
            ...
            try {
                ...
                const healthResult = await makeRequest(`http://localhost:${port}/api/health`);
                ...
                console.log(`💾 Memory usage: ${healthResult.data.memory?.used || 'N/A'}MB\n`);

                // Test /api/health/simple
                console.log('📊 Testing /api/health/simple...');
                const simpleResult = await makeRequest(`http://localhost:${port}/api/health/simple`);
                ...
                const improvement = healthResult.responseTime > simpleResult.responseTime
                    ? Math.round(((healthResult.responseTime - simpleResult.responseTime) / healthResult.responseTime) * 100)
                    : 0;
                ...
                const loadTestHealth = async () => {
                    const promises = Array(10).fill().map(() =>
                        makeRequest(`http://localhost:${port}/api/health`, 2000)
                    );
                    const start = performance.now();
                ...
                console.log(`/api/health (10 concurrent): ${Math.round(healthLoad.totalTime)}ms total`);
                console.log(`/api/health/simple (10 concurrent): ${Math.round(simpleLoad.totalTime)}ms total`);

                const avgHealthTime = healthLoad.results.reduce((sum, r) => sum + r.responseTime, 0) / 10;
                const avgSimpleTime = simpleLoad.results.reduce((sum, r) => sum + r.responseTime, 0) / 10;

                console.log(`Average response times: health=${avgHealthTime.toFixed(2)}ms, simple=${avgSimpleTime.toFixed(2)}ms`);
                ...
                console.log(`- Simple endpoint averages ${avgSimpleTime.toFixed(2)}ms response time`);
                console.log(`- Full endpoint averages ${avgHealthTime.toFixed(2)}ms response time`);
