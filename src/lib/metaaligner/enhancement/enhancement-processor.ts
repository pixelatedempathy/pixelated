...
export class EnhancementProcessor implements IEnhancementProcessor {
  ...
  public async enhance(request: UnifiedProcessingRequest): Promise<UnifiedProcessingResponse> {
    const optimizedContent = this.multiObjectiveOptimization(request.llmOutput.content as string);
