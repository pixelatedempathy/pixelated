...
def verify_tier1_datasets():
    ...
    for s3_path in unique_datasets:
        ...
        if any(indicator in key.lower() for indicator in tier1_indicators):
            tier1_candidates.append(s3_path)  # Keep the full S3 path
    ...
    for s3_path in tier1_candidates[:20]:  # Check top 20 candidates
        try:
            print(f"🔍 Checking: {s3_path}")
            
            # First check if object exists using the full S3 path
            if not loader.object_exists(s3_path):
                print(f"   ❌ Object does not exist")
                continue
            
            # Load a sample to verify accessibility and get size
            try:
                sample_data = loader.load_bytes(s3_path)
                if sample_data:
                    size_mb = len(sample_data) / (1024 * 1024)
                    total_size_mb += size_mb
                    accessible_count += 1
                    found_datasets.append((s3_path, size_mb))
                    ...
                    if s3_path.endswith('.jsonl'):
                        ...
                        sample_lines = []
                        for i, item in enumerate(loader.stream_jsonl(s3_path)):
