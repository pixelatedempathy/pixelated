...
export function usePixelConversationIntegration(
  ...
  const { sessionId, userId, autoInitialize = true, ...pixelConfig } = options
  ...
  const analyzeMessage = useCallback(
    async (
      ...
      try {
        ...
        // Analyze with Pixel
        const response = await integrationRef.current.analyzeConversationTurn(
          message,
          contextType,
        )
...
export function usePixelBiasDetection(
  ...
  const severeBiasCount = biasFlags.filter((f) => f.severity === 'high').length
