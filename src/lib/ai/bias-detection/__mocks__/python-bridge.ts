...
function computeBiasScore(session: TherapeuticSession): number {
  ...
  if (critical > 0) {
    ...
    score = Math.min(0.85, 0.7 + strong * 0.08)
  } else if (medium > 0) {
    score = Math.min(0.7, 0.45 + medium * 0.12)
  } else if (subtle >= 2) {
    score = Math.min(0.5, 0.3 + subtle * 0.08)
  } else if (subtle === 1) {
    score = 0.2
  } else {
    score = 0.05
...
export class PythonBiasDetectionBridge {
  ...
  async runModelLevelAnalysis(session: TherapeuticSession): Promise<any> {
    ...
    return {
      ...
      layer: 'model_level',
      metrics: {
        performance_metrics: { accuracy: 0.9, precision: 0.9, recall: 0.9 },
      },
    }
  }
  ...
  async runInteractiveAnalysis(session: TherapeuticSession): Promise<any> {
    ...
    return {
      biasScore: score,
      layer: 'interactive',
      metrics: { counterfactual_analysis: { bias_detected: score > 0.3 } },
    }
  }
  ...
  async runEvaluationAnalysis(session: TherapeuticSession): Promise<any> {
    ...
    return {
      biasScore: score,
      layer: 'evaluation',
      metrics: { hugging_face_metrics: { bias: score } },
    }
  }
