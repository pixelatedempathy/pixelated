...
const SECURITY_PATTERNS = [
  ...
  { pattern: /\bnew\s+Function\s*\(/, message: 'Use of new Function() detected', safe: false },
  { pattern: /\bsetTimeout\s*\(\s*["'`][^"']*["'`]/, message: 'setTimeout with string argument detected', safe: false },
  { pattern: /\bsetInterval\s*\(\s*["'`][^"']*["'`]/, message: 'setInterval with string argument detected', safe: false },
  { pattern: /innerHTML\s*=\s*[^'"]*$/, message: 'Potential XSS via innerHTML', safe: false },
  { pattern: /document\.write\s*\(/, message: 'Use of document.write() detected', safe: false },
]
...
const EXCLUDED_DIRS = ['node_modules', '.git', 'dist', 'build', '.next', '.astro']
