...
export class PixelConversationIntegration {
  ...
  async analyzeConversationTurn(
    ...
    try {
      const request: PixelInferenceRequest = {
        ...
        conversation_history: this.state.conversationHistory.map((turn) => ({
          role: turn.role,
        ...
        include_metrics:
          this.config.enableBiasDetection || this.config.enableCrisisDetection,
        max_tokens: 200,
      }
  ...
  private updateEQMetrics(eqScores: EQScores): void {
    ...
    this.state.eqMetricsAggregate.socialCognition.push(
      eqScores.social_cognition,
    )
  ...
  private processCrisisDetection(metadata: ConversationMetadata): void {
    ...
    if (crisisSignals.length > 0) {
      ...
      this.state.crisisStatus.interventionType =
        this.determineInterventionType(crisisSignals)
