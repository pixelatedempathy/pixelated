...
export interface ThreatValidationSystem {
  ...
  getValidationHistory(threatId: string, limit?: number): Promise<ThreatValidation[]>;
...
export class ThreatValidationSystemCore extends EventEmitter implements ThreatValidationSystem {
  private redis: Redis;
  private mongoClient: MongoClient;
  private db: Db;
  private validationRules: Map<string, ValidationRule> = new Map();
  private activeValidations: Map<string, ThreatValidation> = new Map();
  private threatIntelligenceCache: Map<string, GlobalThreatIntelligence> = new Map();
  ...
  private async initializeMongoDB(): Promise<void> {
    try {
      this.mongoClient = new MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017/threat_validation');
  ...
  async validateThreat(threat: GlobalThreatIntelligence): Promise<ThreatValidation> {
    try {
      ...
      const indicatorValidation = await this.validateIndicators(threat.indicators);
      validation.results.push(indicatorValidation);
      
      // Step 4: Validate attribution
      if (threat.attribution) {
        const attributionValidation = await this.validateAttribution(threat.attribution);
      ...
      const crossReferenceValidation = await this.crossReferenceWithKnownThreats(threat);
      validation.results.push(crossReferenceValidation);
      
      // Step 8: Calculate overall validation score
      const overallScore = this.calculateOverallValidationScore(validation.results);
      ...
      logger.error('Failed to validate threat:', { error, threatId: threat.threatId });
      this.emit('validation_error', { error, threatId: threat.threatId });
      throw error;
    }
  }

  private async createValidationRecord(threat: GlobalThreatIntelligence): Promise<ThreatValidation> {
  ...
  private async validateThreatStructure(threat: GlobalThreatIntelligence): Promise<ValidationResult> {
    try {
      const issues: string[] = [];
      let score = 100;
      ...
      if (!threat.severity || !['low', 'medium', 'high', 'critical'].includes(threat.severity)) {
        issues.push('Invalid or missing severity');
        score -= 15;
      }
      
      if (threat.confidence === undefined || threat.confidence < 0 || threat.confidence > 1) {
      ...
      if (threat.firstSeen && threat.lastSeen && threat.firstSeen > threat.lastSeen) {
  ...
  async validateIndicators(indicators: ThreatIndicator[]): Promise<ValidationResult> {
    try {
      const issues: string[] = [];
      let score = 100;
      ...
      for (let i = 0; i < indicators.length; i++) {
        ...
        if (indicator.confidence === undefined || indicator.confidence < 0 || indicator.confidence > 1) {
  ...
  private validateIndicatorFormat(indicator: ThreatIndicator): { valid: boolean; error?: string } {
    try {
      ...
      return { valid: false, error: 'Format validation error: ' + error.message };
    }
  }
  ...
  private validateFileHashFormat(hash: string): { valid: boolean; error?: string } {
    ...
    return { valid: false, error: 'Invalid file hash format (expected MD5, SHA1, or SHA256)' };
  }

  private validateEmailFormat(email: string): { valid: boolean; error?: string } {
  ...
  private validateProcessFormat(process: string): { valid: boolean; error?: string } {
  ...
  private async applyCustomValidationRules(threat: GlobalThreatIntelligence): Promise<ValidationResult[]> {
    ...
    for (const rule of this.validationRules.values()) {
      try {
        ...
        logger.error('Custom validation rule failed:', { error, ruleId: rule.ruleId });
        results.push({
          ruleId: rule.ruleId,
          ruleName: rule.name,
          passed: false,
  ...
  private async applyValidationRule(rule: ValidationRule, threat: GlobalThreatIntelligence): Promise<ValidationResult> {
    try {
      const issues: string[] = [];
      let score = 100;
      ...
      for (const condition of rule.conditions) {
        const conditionResult = await this.evaluateValidationCondition(condition, threat);
      ...
      logger.error('Validation rule application failed:', { error, ruleId: rule.ruleId });
      return {
        ruleId: rule.ruleId,
        ruleName: rule.name,
        passed: false,
  ...
  private async evaluateValidationCondition(condition: any, threat: GlobalThreatIntelligence): Promise<{ passed: boolean; message: string }> {
    try {
      ...
      return { passed: false, message: 'Condition evaluation error: ' + error.message };
    }
  }

  private evaluateFieldExistsCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
  ...
  private evaluateFieldValueCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
    const value = this.getNestedValue(threat, condition.field);
    ...
    if (condition.operator === 'equals') {
      ...
      return {
        passed,
        message: passed ? '' : `Field ${condition.field} must equal ${condition.value}`
      };
    }
    ...
    if (condition.operator === 'not_equals') {
      ...
      return {
        passed,
        message: passed ? '' : `Field ${condition.field} must not equal ${condition.value}`
      };
    }
  ...
  private evaluateRegexMatchCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
    const value = this.getNestedValue(threat, condition.field);
    
    if (typeof value !== 'string') {
      return { passed: false, message: `Field ${condition.field} must be a string for regex matching` };
    }
    ...
    return {
      passed,
      message: passed ? '' : `Field ${condition.field} must match pattern ${condition.pattern}`
    };
  }

  private evaluateRangeCheckCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
    const value = this.getNestedValue(threat, condition.field);
    const numValue = Number(value);
    
    if (isNaN(numValue)) {
      return { passed: false, message: `Field ${condition.field} must be a number for range check` };
    }
  ...
  private evaluateWhitelistCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
    const value = this.getNestedValue(threat, condition.field);
  ...
  private evaluateBlacklistCondition(condition: any, threat: GlobalThreatIntelligence): { passed: boolean; message: string } {
    const value = this.getNestedValue(threat, condition.field);
  ...
  private async crossReferenceWithKnownThreats(threat: GlobalThreatIntelligence): Promise<ValidationResult> {
    try {
      const issues: string[] = [];
      let score = 100;
      ...
      const hasWhitelistedIndicators = await this.checkWhitelistedIndicators(threat.indicators);
      ...
      if (similarityScore > 0.9) {
        issues.push('Threat is very similar to existing threats (possible duplicate)');
        score -= 20;
      }
      
      // Check reputation of indicators
      const reputationScore = await this.checkIndicatorReputation(threat.indicators);
  ...
  private async checkKnownFalsePositives(threat: GlobalThreatIntelligence): Promise<boolean> {
    try {
      // Check against known false positive patterns
      const falsePositivesCollection = this.db.collection('known_false_positives');
  ...
  private async checkWhitelistedIndicators(indicators: ThreatIndicator[]): Promise<boolean> {
    try {
  ...
  private async calculateThreatSimilarity(threat: GlobalThreatIntelligence): Promise<number> {
    try {
      ...
      for (const similarThreat of similarThreats) {
        const commonIndicators = similarThreat.indicators.filter((i: ThreatIndicator) => 
          threat.indicators.some(ti => ti.value === i.value)
        );
        
        const similarity = commonIndicators.length / Math.max(similarThreat.indicators.length, threat.indicators.length);
  ...
  private async checkIndicatorReputation(indicators: ThreatIndicator[]): Promise<number> {
    try {
  ...
  private applyValidationWeights(results: ValidationResult[], baseScore: number): number {
    // Critical rules that should heavily impact the score
    const criticalRules = ['structure_validation', 'indicator_validation'];
    const criticalFailures = results.filter(r => 
      criticalRules.includes(r.ruleId) && !r.passed
    );
  ...
  private async storeValidationResult(validation: ThreatValidation): Promise<void> {
    try {
  ...
  private async cacheValidatedThreat(threat: GlobalThreatIntelligence): Promise<void> {
    try {
  ...
  private async sendValidationAlert(validation: ThreatValidation): Promise<void> {
    try {
      const alert = {
        ...
        score: validation.overallScore,
        issues: validation.results.filter(r => !r.passed).map(r => r.issues).flat(),
        timestamp: new Date()
      };
  ...
  async getValidationHistory(threatId: string, limit: number = 50): Promise<ThreatValidation[]> {
    try {
      const validationsCollection = this.db.collection('threat_validations');
  ...
  async updateValidationRule(rule: ValidationRule): Promise<boolean> {
    try {
      ...
      // Update in database
      const rulesCollection = this.db.collection('validation_rules');
      await rulesCollection.replaceOne(
        { ruleId: rule.ruleId },
        rule,
        { upsert: true }
      );
  ...
  private validateValidationRule(rule: ValidationRule): void {
    ...
    if (rule.severity && !['low', 'medium', 'high', 'critical'].includes(rule.severity)) {
      throw new Error('Invalid validation rule: severity must be low, medium, high, or critical');
    }
  }
  ...
  private async calculateAverageValidationTime(): Promise<number> {
    try {
      ...
      for (const validation of completedValidations) {
        const timeDiff = validation.completedAt.getTime() - validation.createdAt.getTime();
        totalTime += timeDiff;
      }
  ...
  private async calculateFalsePositives(): Promise<number> {
    try {
      ...
      const falsePositives = await validationsCollection
        .countDocuments({ 
          isValid: false,
  ...
  private async calculateFalseNegatives(): Promise<number> {
    try {
      ...
      const falseNegatives = await validationsCollection
        .countDocuments({ 
          isValid: true,
  ...
  async getHealthStatus(): Promise<HealthStatus> {
    try {
      ...
      const successRate = metrics.totalValidations > 0 ? 
        (metrics.validThreats / metrics.totalValidations) * 100 : 0;
...
export { ThreatValidationSystemCore };