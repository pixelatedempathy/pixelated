...
- **Guidance for Copilot:**
  ...
  - Guide on setting explicit `permissions` for `G_TOKEN` to adhere to the
...
- **Deeper Dive:**
  - **`uses`:** Referencing marketplace actions (e.g., `actions/checkout@v5`,
...
- **Principle:** Grant only the necessary permissions to the `G_TOKEN` for your
  workflows, reducing the blast radius in case of compromise.
- **Deeper Dive:**
  - **Default Permissions:** By default, the `G_TOKEN` has broad permissions.
    This should be explicitly restricted.
...
- **Guidance for Copilot:**
  ...
  - Provide a clear mapping of common workflow needs to specific `G_TOKEN`
    permissions.
...
- **Guidance for Copilot:**
  - Use `actions/checkout@v5` with `fetch-depth: 1` as the default for most
...
- [ ] **Security Considerations:**
  ...
  - Is `G_TOKEN` permission scope explicitly defined and limited to the minimum
    necessary access (`contents: read` as a baseline)?
...
- **Root Causes:** `G_TOKEN` lacking necessary permissions, incorrect
