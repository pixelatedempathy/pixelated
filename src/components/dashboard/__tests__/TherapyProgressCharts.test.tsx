...
describe("TherapyProgressCharts", () => {
  ...
  it("renders session progress timeline with correct data", () => {
    ...
    const timelineChart = screen.getByText("Session Progress Timeline").closest('div');
  ...
  it("renders skill development radar chart", () => {
    ...
    const radarChart = screen.getByText("Skill Development Radar").closest('div');
  ...
  it("renders session comparison with trend data", () => {
    ...
    const comparisonChart = screen.getByText("Session Comparison").closest('div');
  ...
  it("renders skill improvement timeline", () => {
    ...
    const timelineChart = screen.getByText("Skill Practice Timeline").closest('div');
    expect(timelineChart).toBeInTheDocument();
  ...
  it("handles missing comparative data gracefully", () => {
    ...
    expect(screen.getByText("Not enough session data for comparison")).toBeInTheDocument();
  });
  ...
  it("handles missing previous session in comparison", () => {
    ...
    expect(screen.getByText("Previous session data not available")).toBeInTheDocument();
  });
  ...
  it("applies correct styling classes", () => {
    ...
    const comparisonGrid = screen.getByText("Session Comparison").closest('div')?.parentElement;
    expect(comparisonGrid).toHaveClass("grid", "grid-cols-1", "md:grid-cols-2", "gap-6");
  });

  it("renders with custom className", () => {
    render(<TherapyProgressCharts data={mockData} className="custom-chart-class" />);
