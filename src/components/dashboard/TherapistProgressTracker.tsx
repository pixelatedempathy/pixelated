...
export function TherapistProgressTracker({ session, className }: TherapistProgressTrackerProps) {
  ...
  return (
    <div className={cn('space-y-6', className)} aria-label="Therapist Progress Tracker" role="region" tabIndex={0}>
      {/* Session Overview */}
      <section className="bg-muted rounded-md p-4 focus:outline-none focus:ring-2 focus:ring-primary" aria-labelledby="overview-heading" tabIndex={0}>
      ...
      {/* Overall Progress */}
      <section className="bg-muted rounded-md p-4 focus:outline-none focus:ring-2 focus:ring-primary" aria-labelledby="progress-heading" tabIndex={0}>
        <div className="flex items-center justify-between">
        ...
        {expandedSections['progress'] && <ProgressBar value={session.progress} label="Session Completion" />}
      </section>

      {/* Skill Development */}
      <section className="bg-muted rounded-md p-4 focus:outline-none focus:ring-2 focus:ring-primary" aria-labelledby="skills-heading" tabIndex={0}>
        <div className="flex items-center justify-between">
        ...
        {expandedSections['skills'] && (
          <div className="space-y-3">
            {skillsLoading && <div className="text-sm text-muted-foreground">Loading skills…</div>}

            {skillsError && <div className="text-sm text-red-600">Failed to load skills: {skillsError.message}</div>}

            {!skillsLoading && !skillsError && (!effectiveSkills || effectiveSkills.length === 0) && (
              <div className="text-sm text-muted-foreground">No skill progress available for this session.</div>
            )}

            {!skillsLoading && !skillsError && effectiveSkills && effectiveSkills.length > 0 && (
              <div className="space-y-2" role="list">
                {effectiveSkills.map((skill: SkillProgress, index: number) => (
                  <div
                    key={index}
      ...
      {/* Session Notes */}
      <section className="bg-muted rounded-md p-4 focus:outline-none focus:ring-2 focus:ring-primary" aria-labelledby="notes-heading" tabIndex={0}>
        <div className="flex items-center justify-between">
        ...
        {expandedSections['notes'] && (
          <div className="text-sm">
            {session.notes ? <p>{session.notes}</p> : <p className="text-muted-foreground italic">No notes available for this session.</p>}
          </div>
        )}
      </section>
    </div>
