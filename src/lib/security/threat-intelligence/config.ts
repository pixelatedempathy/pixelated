...
const baseConfig = {
  ...
  regions: {
    ...
    secondary: (process.env.SECONDARY_REGIONS || 'eu-west-1,ap-southeast-1').split(','),
...
export const globalThreatIntelConfig: GlobalThreatIntelligenceNetworkConfig = {
  ...
  regions: {
    us_east_1: {
      ...
      endpoint: process.env.US_EAST_ENDPOINT || 'https://api-us-east.pixelated.com',
      priority: 1,
    ...
    eu_west_1: {
      name: 'Europe (Ireland)',
      endpoint: process.env.EU_WEST_ENDPOINT || 'https://api-eu-west.pixelated.com',
      priority: 2,
      capabilities: ['full_processing', 'ai_analysis', 'correlation'],
    ...
    ap_southeast_1: {
      ...
      endpoint: process.env.AP_SOUTHEAST_ENDPOINT || 'https://api-ap-southeast.pixelated.com',
      priority: 3,
      capabilities: ['full_processing', 'ai_analysis', 'correlation'],
  ...
  propagation: {
    ...
    priority_levels: (process.env.PROPAGATION_PRIORITIES || 'critical,high,medium,low').split(','),
  },
  health_check: {
...
export const edgeThreatDetectionConfig: EdgeThreatDetectionSystemConfig = {
  ...
  edge_locations: baseConfig.regions.edge_locations.map((location, index) => ({
    id: `edge-${index + 1}`,
    region: baseConfig.regions.secondary[index % baseConfig.regions.secondary.length] || baseConfig.regions.primary,
  ...
  ml_models: {
    anomaly_detection: {
      ...
      model_path: process.env.ANOMALY_MODEL_PATH || '/models/anomaly_detection.tf',
    ...
    threat_classification: {
      ...
      model_path: process.env.CLASSIFICATION_MODEL_PATH || '/models/threat_classifier.tf',
      confidence_threshold: parseFloat(process.env.CLASSIFICATION_THRESHOLD || '0.8'),
      classes: (process.env.THREAT_CLASSES || 'malware,phishing,ddos,data_breach').split(','),
    },
    clustering: {
      enabled: process.env.CLUSTERING_ENABLED !== 'false',
      model_path: process.env.CLUSTERING_MODEL_PATH || '/models/threat_clustering.tf',
      num_clusters: parseInt(process.env.NUM_CLUSTERS || '10'),
    },
    prediction: {
      enabled: process.env.PREDICTION_ENABLED !== 'false',
      model_path: process.env.PREDICTION_MODEL_PATH || '/models/threat_prediction.tf',
...
export const threatCorrelationConfig: ThreatCorrelationEngineConfig = {
  ...
  correlation_algorithms: {
    temporal: {
      ...
      similarity_threshold: parseFloat(process.env.TEMPORAL_SIMILARITY || '0.7'),
    },
    spatial: {
    ...
    behavioral: {
      ...
      pattern_similarity_threshold: parseFloat(process.env.BEHAVIORAL_SIMILARITY || '0.8'),
    ...
    attribution: {
      ...
      confidence_threshold: parseFloat(process.env.ATTRIBUTION_CONFIDENCE || '0.6'),
      max_attribution_attempts: parseInt(process.env.MAX_ATTRIBUTION_ATTEMPTS || '5'),
    },
  },
  ml_models: {
    similarity_detection: {
      enabled: process.env.SIMILARITY_DETECTION_ENABLED !== 'false',
      model_path: process.env.SIMILARITY_MODEL_PATH || '/models/similarity_detection.tf',
      threshold: parseFloat(process.env.SIMILARITY_THRESHOLD || '0.75'),
    },
    pattern_recognition: {
      enabled: process.env.PATTERN_RECOGNITION_ENABLED !== 'false',
      model_path: process.env.PATTERN_MODEL_PATH || '/models/pattern_recognition.tf',
    ...
    statistical_analysis: {
      ...
      correlation_methods: (process.env.CORRELATION_METHODS || 'pearson,spearman,kendall').split(','),
    },
  },
...
export const threatResponseConfig: AutomatedThreatResponseOrchestratorConfig = {
  ...
  response_strategies: {
    automatic: {
      ...
      confidence_threshold: parseFloat(process.env.AUTO_RESPONSE_CONFIDENCE || '0.9'),
      severity_levels: (process.env.AUTO_RESPONSE_SEVERITIES || 'critical,high').split(','),
    ...
    semi_automatic: {
      ...
      confidence_threshold: parseFloat(process.env.SEMI_AUTO_CONFIDENCE || '0.7'),
    ...
    manual: {
      ...
      escalation_threshold: parseFloat(process.env.MANUAL_ESCALATION_THRESHOLD || '0.5'),
      notification_channels: (process.env.MANUAL_NOTIFICATION_CHANNELS || 'email,slack').split(','),
    },
  },
  integration_apis: {
    ...
    notification_api: process.env.NOTIFICATION_API || 'https://notify.internal/api',
  },
  response_actions: {
    block_ip: { enabled: process.env.BLOCK_IP_ENABLED !== 'false', timeout_minutes: parseInt(process.env.BLOCK_IP_TIMEOUT || '60') },
    quarantine_file: { enabled: process.env.QUARANTINE_FILE_ENABLED !== 'false', scan_before_release: process.env.SCAN_BEFORE_RELEASE === 'true' },
    disable_user: { enabled: process.env.DISABLE_USER_ENABLED !== 'false', require_approval: process.env.DISABLE_USER_APPROVAL === 'true' },
    isolate_system: { enabled: process.env.ISOLATE_SYSTEM_ENABLED !== 'false', duration_minutes: parseInt(process.env.ISOLATION_DURATION || '120') },
    notify_stakeholders: { enabled: process.env.NOTIFY_STAKEHOLDERS_ENABLED !== 'false', urgency_levels: (process.env.NOTIFICATION_URGENCY || 'critical,high').split(',') },
  },
  rollback: {
...
export const threatHuntingConfig: ThreatHuntingSystemConfig = {
  ...
  hunt_templates: [
    {
      ...
      query: {
        type: 'kql',
        query: 'network_connections | where bytes_sent > 1000000 or bytes_received > 1000000',
      },
      scope: {
        regions: ['global'],
        systems: ['all'],
        time_range: { start: new Date(Date.now() - 24 * 60 * 60 * 1000), end: new Date() },
    ...
    },
    {
      ...
      query: {
        type: 'yara',
        query: 'rule malware_detection { strings: $a = "malware" condition: $a }',
      },
      scope: {
        regions: ['global'],
        systems: ['endpoints'],
        time_range: { start: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), end: new Date() },
  ...
  ai_assistance: {
    ...
    confidence_threshold: parseFloat(process.env.HUNTING_AI_CONFIDENCE || '0.7'),
  },
  execution_limits: {
    max_concurrent_hunts: parseInt(process.env.MAX_CONCURRENT_HUNTS || '10'),
    max_findings_per_hunt: parseInt(process.env.MAX_FINDINGS_PER_HUNT || '1000'),
  ...
  integration_apis: {
    ...
    network_monitoring_api: process.env.HUNTING_NETWORK_API || 'https://network-monitor.internal/api',
    log_aggregation_api: process.env.HUNTING_LOG_API || 'https://logs.internal/api',
  },
};
...
export const externalFeedConfig: ExternalThreatFeedIntegrationConfig = {
  ...
  feeds: [
    {
      ...
      endpoint: process.env.MISP_ENDPOINT || 'https://www.circl.lu/doc/misp/feed-osint',
      authentication: {
        type: 'none',
        credentials: {},
      },
  ...
  integration_apis: {
    stix_taxii_client: process.env.STIX_TAXII_CLIENT_API || 'https://taxii.example.com/api',
    misp_client: process.env.MISP_CLIENT_API || 'https://misp.example.com/api',
    threat_intel_platform: process.env.THREAT_INTEL_PLATFORM_API || 'https://platform.example.com/api',
  },
};
...
export const threatValidationConfig: ThreatValidationSystemConfig = {
  ...baseConfig,
  validation_settings: {
    max_concurrent_validations: parseInt(process.env.MAX_CONCURRENT_VALIDATIONS || '10'),
    validation_timeout: parseInt(process.env.VALIDATION_TIMEOUT || '300000'), // 5 minutes
    auto_validation_threshold: parseFloat(process.env.AUTO_VALIDATION_THRESHOLD || '0.8'),
    human_review_threshold: parseFloat(process.env.HUMAN_REVIEW_THRESHOLD || '0.6'),
    quality_gates: [
      {
        name: 'high_accuracy',
        ...
        notification_config: {
          enabled: true,
          channels: ['email'],
          recipients: (process.env.QUALITY_GATE_RECIPIENTS || 'security@pixelated.com').split(','),
          templates: {
            approval: 'Threat {threat_id} automatically approved with score {score}',
          },
        },
      },
      {
        ...
        notification_config: {
          ...
          recipients: (process.env.LOW_CONFIDENCE_RECIPIENTS || 'analysts@pixelated.com').split(','),
          templates: {
            review: 'Threat {threat_id} requires manual review - confidence: {score}',
          },
        },
      },
    ],
  ...
  ai_assistance: {
    ...
    confidence_threshold: parseFloat(process.env.VALIDATION_AI_CONFIDENCE || '0.7'),
  },
  compliance: {
    enabled_standards: (process.env.ENABLED_COMPLIANCE_STANDARDS || 'STIX,TAXII,MISP,HIPAA').split(','),
    audit_logging: process.env.COMPLIANCE_AUDIT_LOGGING !== 'false',
    data_retention_days: parseInt(process.env.COMPLIANCE_RETENTION_DAYS || '2555'), // 7 years
  },
};
...
export function getThreatIntelConfig(component: string): unknown {
  ...
  return (configMap as Record<string, unknown>)[component] || threatIntelligenceConfig;
}

// Environment validation
export function validateThreatIntelConfig(): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (threatIntelDatabaseConfig.stix_taxii.enabled && !threatIntelDatabaseConfig.stix_taxii.taxii_username) {
