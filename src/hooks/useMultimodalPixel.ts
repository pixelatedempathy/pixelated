...
export function useMultimodalPixel(
            ...
            try {
                ...
                ws.onmessage = (event) => {
                    try {
                        ...
                        setState((prev) => ({
                            ...prev,
                            streamError: err instanceof Error ? err.message : 'Malformed message',
                            streamStatus: 'error',
                        }))
                    }
                }
                ...
                setState((prev) => ({
                    ...prev,
                    streamError: err instanceof Error ? err.message : 'Stream init failed',
                    streamStatus: 'error',
                }))
            }
        },
    ...
    const sendTextToStream = useCallback((text: string) => {
        if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return
        wsRef.current.send(
            JSON.stringify({
                type: 'text',
                text,
                contextType: streamConfigRef.current.contextType || defaultContextType,
                sessionId: streamConfigRef.current.sessionId,
            }),
        )
    }, [defaultContextType])

    const sendChunkToStream = useCallback(async (chunk: Blob, mimeType = 'audio/webm') => {
        if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return
        ...
        ({ text, sessionId, contextType }: MultimodalStreamConfig = {}) => {
            ...
            wsRef.current.send(
                JSON.stringify({
                    ...
                    contextType: contextType || streamConfigRef.current.contextType || defaultContextType,
                }),
            )
        },
        [defaultContextType],
    ...
    const infer = useCallback(
        async (args: MultimodalInferArgs) => {
