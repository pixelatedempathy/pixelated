...
export class GlobalTrafficRoutingManager extends EventEmitter {
   ...
   */
  async routeTraffic(userLocation: { latitude: number; longitude: number }, 
                    requestType: string = 'api',
                    complianceRequirements: string[] = []): Promise<RoutingDecision> {
    if (!this.isInitialized) {
      throw new Error('Traffic routing manager not initialized');
    }

    try {
      const cacheKey = this.generateCacheKey(userLocation, requestType, complianceRequirements);
      ...
      const decision = await this.makeRoutingDecision(userLocation, requestType, complianceRequirements);
      ...
      logger.error('Traffic routing failed', { error, userLocation, requestType });
      
      // Fallback to default region
      const fallbackDecision = this.getFallbackDecision();
      this.emit('traffic-routing-failed', { error: error.message, fallback: fallbackDecision.target.regionId });
   ...
   */
  private generateCacheKey(userLocation: { latitude: number; longitude: number }, 
                           requestType: string, 
   ...
   */
  private async makeRoutingDecision(userLocation: { latitude: number; longitude: number }, 
                                   requestType: string, 
                                   complianceRequirements: string[]): Promise<RoutingDecision> {
    switch (this.config.strategy) {
      case 'latency-based':
        return this.makeLatencyBasedDecision(userLocation, requestType, complianceRequirements);
      case 'health-based':
        return this.makeHealthBasedDecision(userLocation, requestType, complianceRequirements);
      case 'compliance-based':
        return this.makeComplianceBasedDecision(userLocation, requestType, complianceRequirements);
      case 'weighted-round-robin':
        return this.makeWeightedRoundRobinDecision(userLocation, requestType, complianceRequirements);
      default:
   ...
   */
  private async makeLatencyBasedDecision(userLocation: { latitude: number; longitude: number }, 
                                        requestType: string, 
                                        complianceRequirements: string[]): Promise<RoutingDecision> {
    ...
    const filteredTargets = targetsWithLatency.filter(t => t.latency <= this.config.latencyThreshold);
   ...
   */
  private async makeHealthBasedDecision(userLocation: { latitude: number; longitude: number }, 
                                       requestType: string, 
                                       complianceRequirements: string[]): Promise<RoutingDecision> {
    const eligibleTargets = this.getEligibleTargets(complianceRequirements);
    ...
    const healthyTargets = eligibleTargets.filter(t => t.healthScore >= this.config.healthThreshold);
   ...
   */
  private async makeComplianceBasedDecision(userLocation: { latitude: number; longitude: number }, 
                                           requestType: string, 
                                           complianceRequirements: string[]): Promise<RoutingDecision> {
    const eligibleTargets = this.getEligibleTargets(complianceRequirements);
   ...
   */
  private async makeWeightedRoundRobinDecision(userLocation: { latitude: number; longitude: number }, 
                                              requestType: string, 
                                              complianceRequirements: string[]): Promise<RoutingDecision> {
    const eligibleTargets = this.getEligibleTargets(complianceRequirements);
    ...
    selectedTarget.latency = await this.calculateLatency(userLocation, selectedTarget.target);
   ...
   */
  private meetsComplianceRequirements(target: RouteTarget, requirements: string[]): boolean {
    if (requirements.length === 0) return true;
    
    const region = this.loadRegionConfiguration().find(r => r.id === target.regionId);
   ...
   */
  private meetsAllComplianceRequirements(target: RouteTarget, requirements: string[]): boolean {
    const region = this.loadRegionConfiguration().find(r => r.id === target.regionId);
    if (!region) return false;
   ...
   */
  private async calculateLatency(userLocation: { latitude: number; longitude: number }, 
                                 target: RouteTarget): Promise<number> {
   ...
   */
  private getRegionCoordinates(location: string): { latitude: number; longitude: number } {
  ...
  private collectMetrics(): void {
    try {
      ...
      for (const region in this.metrics.errorRateByRegion) {
        ...
        this.metrics.errorRateByRegion[region] = requests > 0 ? errors / requests : 0;
      }
  ...
  updateTargetHealth(regionId: string, healthScore: number): void {
    ...
    if (target) {
      ...
      logger.info(`Updated health score for region: ${regionId}`, { healthScore });
