...
class IntegrationTestRunner {
  ...
  private async checkServiceHealth(): Promise<void> {
    const services = [
      { name: 'Database', url: '/api/health', check: (data: any) => data.services?.database === 'connected' },
      { name: 'Redis', url: '/api/health', check: (data: any) => data.services?.redis === 'connected' },
      { name: 'Bias Detection', url: '/api/health', check: (data: any) => data.services?.bias_detection === 'operational' },
    ]
    
    for (const service of services) {
      if (this.config.services[service.name.toLowerCase() as keyof typeof this.config.services]) {
        await this.verifyServiceHealth(service)
      }
    }
  }

  private async verifyServiceHealth(service: { name: string; url: string; check: (data: any) => boolean }): Promise<void> {
    const maxRetries = 5
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`⚠️  ${service.name} health check failed (attempt ${attempt}/${maxRetries}):`, error.message)
      }
    ...
    throw new Error(`${service.name} service is not healthy after ${maxRetries} attempts`)
  }
  ...
  private async generateReport(): Promise<void> {
    ...
    const reportPath = join(__dirname, '../../test-results/integration/integration-report.json')
  ...
  private async generatePerformanceReport(): Promise<Record<string, any>> {
    ...
    const benchmarks = [
      { name: 'Health Check', url: '/api/health/simple', threshold: this.config.performanceThresholds.healthCheck },
      { name: 'Bias Analysis', url: '/api/bias-analysis/analyze', method: 'POST', threshold: this.config.performanceThresholds.biasAnalysis },
      { name: 'Dashboard', url: '/api/bias-analysis/dashboard', threshold: this.config.performanceThresholds.dashboard },
    ]
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          const options: any = {
            ...
            headers: benchmark.method === 'POST' ? { 'Content-Type': 'application/json' } : {},
          }
          ...
          if (benchmark.method === 'POST') {
            options.body = JSON.stringify({
              ...
              demographics: { age: '25-35', gender: 'female', ethnicity: 'hispanic', primaryLanguage: 'en' },
            })
          }
          
          const response = await fetch(`http://localhost:3000${benchmark.url}`, options)
          const duration = Date.now() - startTime
