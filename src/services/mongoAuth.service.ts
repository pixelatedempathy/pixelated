...
if (typeof window === 'undefined') {
  MongoAuthServiceImpl = class MongoAuthService {
    ...
    async createUser(email: string, password: string, role: 'admin' | 'user' | 'therapist' = 'user'): Promise<User> {
    ...
    async refreshSession(accessToken: string): Promise<AuthResult> {
      ...
      try {
        ...
        const existingSession = await sessionsCollection.findOne({ userId: user._id, expiresAt: { $gt: new Date() } });
        ...
        const newAccessToken = this.generateToken({ userId: user._id.toString(), email: user.email, role: user.role });
        const updatedSession = await sessionsCollection.findOne({ _id: existingSession._id });
