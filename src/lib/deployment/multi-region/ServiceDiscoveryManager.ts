...
export class ServiceDiscoveryManager extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error('Failed to initialize ServiceDiscoveryManager', { error });
      throw error;
    }
  }
  ...
  private async initializeDiscoveryBackends(): Promise<void> {
    ...
    for (const region of regions) {
      try {
        ...
        this.logger.error(`Failed to initialize discovery backends for region: ${region}`, { error });
        throw error;
      }
    }
  }
  ...
  private async initializeConsul(region: string, config: any): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to initialize Consul for region: ${region}`, { error });
      throw error;
    }
  }
  ...
  private async initializeEtcd(region: string, config: any): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to initialize etcd for region: ${region}`, { error });
      throw error;
    }
  }
   ...
   */
  private async initializeZookeeper(region: string, config: any): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to initialize ZooKeeper for region: ${region}`, { error });
      throw error;
    }
  }
    ...
    this.healthMonitor.registerCheck('service-discovery', async () => {
      try {
        ...
        if (healthyBackends.length === 0) {
          return { status: 'unhealthy', message: 'No healthy discovery backends' };
        }
        
        if (healthyBackends.length < totalBackends * 0.5) {
          return { status: 'degraded', message: `Only ${healthyBackends.length}/${totalBackends} backends healthy` };
        }
        
        return { status: 'healthy', message: `${healthyBackends.length}/${totalBackends} backends healthy` };
      } catch (error) {
        return { status: 'unhealthy', message: `Service discovery check failed: ${error.message}` };
      }
    });
  }
  ...
  private async getHealthyBackends(): Promise<DiscoveryBackend[]> {
    ...
    for (const backend of this.discoveryBackends.values()) {
      try {
        ...
        this.logger.warn(`Backend health check failed`, { backend: backend.type, region: backend.region, error });
      }
    }
   ...
   */
  private async checkBackendHealth(backend: DiscoveryBackend): Promise<boolean> {
    try {
      switch (backend.type) {
        case 'consul': {
      ...
      this.healthMonitor.registerCheck(`service-${serviceName}`, async () => {
        try {
          ...
          if (healthyInstances.length === 0) {
            return { status: 'unhealthy', message: `No healthy instances for ${serviceName}` };
          }
          
          if (healthyInstances.length < totalInstances * 0.5) {
            return { status: 'degraded', message: `Only ${healthyInstances.length}/${totalInstances} instances healthy for ${serviceName}` };
          }
          
          return { status: 'healthy', message: `${healthyInstances.length}/${totalInstances} instances healthy for ${serviceName}` };
        } catch (error) {
          return { status: 'unhealthy', message: `Service health check failed for ${serviceName}: ${error.message}` };
        }
      });
    }
  }
   ...
   */
  private async registerWithBackend(backend: DiscoveryBackend, instance: ServiceInstance): Promise<void> {
    try {
      switch (backend.type) {
        case 'consul': {
        ...
        case 'zookeeper': {
          await this.registerWithZookeeper(backend.client as ZooKeeperClient, instance);
          break;
        }
      }
    } catch (error) {
   ...
   */
  private async registerWithConsul(consul: Consul, instance: ServiceInstance): Promise<void> {
    const service = {
      ID: instance.id,
      Name: instance.name,
   ...
   * Register with etcd
   */
  private async registerWithEtcd(etcd: Etcd3, instance: ServiceInstance): Promise<void> {
   ...
   */
  private async registerWithZookeeper(zk: ZooKeeperClient, instance: ServiceInstance): Promise<void> {
    ...
    await zk.create(instancePath, Buffer.from(data), ZooKeeperClient.CreateMode.EPHEMERAL);
  }

  /**
   * Create ZooKeeper path recursively
   */
  private async createZookeeperPath(zk: ZooKeeperClient, path: string): Promise<void> {
    ...
    for (const part of parts) {
      currentPath += `/${part}`;
      try {
        await zk.create(currentPath, Buffer.from(''), ZooKeeperClient.CreateMode.PERSISTENT);
      } catch (error: any) {
   ...
   */
  async discoverService(serviceName: string, options: DiscoveryOptions = {}): Promise<ServiceInstance[]> {
    try {
      ...
      if (cached && !this.isCacheExpired(cached)) {
        this.logger.debug('Returning cached service instances', { serviceName, count: cached.instances.length });
      ...
      this.logger.error('Service discovery failed', { serviceName, options, error });
      
      // Return cached results if available, even if expired
      const cacheKey = this.getCacheKey(serviceName, options);
      const cached = this.serviceCache.get(cacheKey);
      if (cached) {
        this.logger.warn('Returning expired cache due to discovery failure', { serviceName });
        return cached.instances;
      }
   ...
   */
  private async discoverFromBackends(serviceName: string, options: DiscoveryOptions): Promise<ServiceInstance[]> {
    const backends = await this.getHealthyBackends();
   ...
   */
  private async discoverFromBackend(backend: DiscoveryBackend, serviceName: string, options: DiscoveryOptions): Promise<ServiceInstance[]> {
    try {
      switch (backend.type) {
        case 'consul': {
          return await this.discoverFromConsul(backend.client as Consul, serviceName, options);
        }
        case 'etcd': {
          return await this.discoverFromEtcd(backend.client as Etcd3, serviceName, options);
        }
        case 'zookeeper': {
          return await this.discoverFromZookeeper(backend.client as ZooKeeperClient, serviceName, options);
        }
        default: {
          return [];
        }
   ...
   */
  private async discoverFromConsul(consul: Consul, serviceName: string, options: DiscoveryOptions): Promise<ServiceInstance[]> {
   ...
   * Discover from etcd
   */
  private async discoverFromEtcd(etcd: Etcd3, serviceName: string, options: DiscoveryOptions): Promise<ServiceInstance[]> {
   ...
   */
  private async discoverFromZookeeper(zk: ZooKeeperClient, serviceName: string, options: DiscoveryOptions): Promise<ServiceInstance[]> {
   ...
   */
  private filterInstances(instances: ServiceInstance[], options: DiscoveryOptions): ServiceInstance[] {
    return instances.filter(instance => {
      ...
      if (options.tags && options.tags.length > 0) {
        const hasAllTags = options.tags.every(tag => instance.tags.includes(tag));
   ...
   */
  private sortInstances(instances: ServiceInstance[], options: DiscoveryOptions): ServiceInstance[] {
  ...
  private getCacheKey(serviceName: string, options: DiscoveryOptions): string {
    ...
    if (options.tags && options.tags.length > 0) keyParts.push(`tags:${options.tags.join(',')}`);
   ...
   */
  private async sendHeartbeatToBackend(backend: DiscoveryBackend, instance: ServiceInstance): Promise<void> {
    try {
      switch (backend.type) {
        case 'consul': {
  ...
  private async cleanupExpiredServices(): Promise<void> {
    try {
      ...
      for (const [serviceName, instances] of this.serviceRegistry) {
        ...
        if (removedCount > 0) {
          this.logger.info(`Removed ${removedCount} expired instances for ${serviceName}`);
   ...
   */
  async getServiceInstance(serviceName: string, options: DiscoveryOptions = {}): Promise<ServiceInstance | null> {
    try {
      ...
      this.logger.error('Failed to get service instance', { serviceName, options, error });
      throw error;
    }
  }
   ...
   */
  async watchService(serviceName: string, callback: (instances: ServiceInstance[]) => void, options: DiscoveryOptions = {}): Promise<() => void> {
    try {
      ...
      this.logger.error('Failed to start service watch', { serviceName, options, error });
      throw error;
    }
  }
  ...
  async getServiceStats(serviceName?: string): Promise<ServiceStats> {
    try {
      ...
      this.logger.error('Failed to get service statistics', { serviceName, error });
      throw error;
    }
  }
    ...
    try {
      ...
      for (const [key, entry] of this.serviceCache) {
        if (key.startsWith(serviceName)) {
          const filteredInstances = entry.instances.filter(i => i.id !== instanceId);
      ...
      this.logger.info('Service deregistered successfully', { serviceName, instanceId });
      
      this.emit('serviceDeregistered', { serviceName, instanceId });
    } catch (error) {
      this.logger.error('Failed to deregister service', { serviceName, instanceId, error });
      throw error;
    }
  }
   ...
   */
  private async deregisterFromBackend(backend: DiscoveryBackend, serviceName: string, instanceId: string): Promise<void> {
    try {
      switch (backend.type) {
        case 'consul': {
          const consul = backend.client as Consul;
...
class LoadBalancer {
  ...
  selectInstance(instances: ServiceInstance[], options: DiscoveryOptions = {}): ServiceInstance | null {
  ...
  private geoProximity(instances: ServiceInstance[], clientRegion?: string): ServiceInstance {
