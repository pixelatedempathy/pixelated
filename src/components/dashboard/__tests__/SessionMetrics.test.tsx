...
describe("SessionMetrics", () => {
  ...
  it("applies correct styling classes", () => {
    ...
    const metricItems = screen.getAllByText(/Total Sessions|Avg Progress|Completed|Avg Duration/);
    metricItems.forEach(item => {
      const parent = item.closest('div');
      if (parent) {
        expect(parent).toHaveClass("bg-muted", "rounded-md", "p-2", "flex", "flex-col", "items-center");
      }
    });
  });
