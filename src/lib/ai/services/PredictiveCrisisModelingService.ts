...
export class PredictiveCrisisModelingService {
  ...
  async predictCrisisRisk(
    ...
    try {
      ...
      const emotionalRisk = this.calculateEmotionalRisk(emotionHistory, riskFactors.emotional)
      const behavioralRisk = this.calculateBehavioralRisk(riskFactors.behavioral)
      const historicalRisk = this.calculateHistoricalRisk(riskFactors.historical)
      ...
      const primaryRiskFactors = this.identifyPrimaryRiskFactors(riskFactors, compositeRisk)
      const protectiveFactors = this.identifyProtectiveFactors(riskFactors)

      // Determine intervention windows
      const interventionWindow = this.calculateInterventionWindow(riskLevel, timeframe)

      // Generate escalation triggers
      const escalationTriggers = this.generateEscalationTriggers(riskLevel, riskFactors)
  ...
  generateTemporalRiskModel(
    ...
    const shortTerm = {
      ...
      next6Hours: this.calculateHourlyRisk(baseRisk, riskFactors.temporal) * 1.2,
      next24Hours: this.calculateHourlyRisk(baseRisk, riskFactors.temporal) * 1.5
    }
  ...
  private calculateEmotionalRisk(
    ...
    const intensityVariance = this.calculateVariance(emotionalFactors.intensityTrend)
    const volatilityScore = Math.min(intensityVariance / 100, 1)

    // Factor in negative emotion persistence
    const negativeEmotionWeight = Math.min(emotionalFactors.negativeEmotionDuration / 7, 1)

    // Consider emotional regulation capacity
    const regulationDeficit = Math.max(0, 1 - emotionalFactors.emotionalRegulationCapacity)

    // Recent high-intensity emotions
    const recentIntensity = emotionHistory.slice(-3).reduce((sum, emotion) => 
      sum + emotion.intensity, 0) / Math.min(3, emotionHistory.length)

    return (volatilityScore * 0.3) + (negativeEmotionWeight * 0.3) + 
           (regulationDeficit * 0.25) + (recentIntensity * 0.15)
  }
  ...
  private calculateBehavioralRisk(
    ...
    const communicationRisk = behavioralFactors.communicationPatterns.includes('withdrawal') ? 0.8 :
                            behavioralFactors.communicationPatterns.includes('agitation') ? 0.7 : 0.3

    // Social isolation indicators
    const isolationRisk = Math.min(behavioralFactors.isolationIndicators / 10, 1)

    // Activity level changes
    const activityRisk = Math.abs(behavioralFactors.activityChanges - 1) * 0.5

    return (engagementRisk * 0.4) + (communicationRisk * 0.3) + 
           (isolationRisk * 0.2) + (activityRisk * 0.1)
  }
  ...
  private calculateHistoricalRisk(
    ...
    const timeFactor = historicalFactors.timesSinceLastCrisis > 90 ? 0.2 : 
                      historicalFactors.timesSinceLastCrisis > 30 ? 0.5 : 0.8

    // Treatment compliance
    const complianceRisk = Math.max(0, 1 - historicalFactors.treatmentCompliance)

    // Support system strength (inverse)
    const supportRisk = Math.max(0, 1 - historicalFactors.supportSystemStrength)

    return (crisisFrequency * 0.4) + (timeFactor * 0.25) + 
           (complianceRisk * 0.2) + (supportRisk * 0.15)
  }
  ...
  private calculateTemporalRisk(
    ...
    const sessionGapRisk = Math.min(temporalFactors.daysSinceLastSession / 14, 1)

    // Time of day risk (higher at night/early morning)
    const timeRisk = (temporalFactors.timeOfDay >= 22 || temporalFactors.timeOfDay <= 6) ? 0.7 : 0.3

    // Seasonal factors
    const seasonalRisk = temporalFactors.seasonalFactors

    // Anniversary dates proximity
    const anniversaryRisk = temporalFactors.anniversaryDates.length > 0 ? 0.6 : 0.1

    return (sessionGapRisk * 0.4) + (timeRisk * 0.3) + 
           (seasonalRisk * 0.2) + (anniversaryRisk * 0.1)
  }

  private determineRiskLevel(compositeRisk: number): CrisisPrediction['riskLevel'] {
  ...
  private predictTimeframe(
    ...
    const urgencyScore = 
      (riskFactors.emotional.intensityTrend.slice(-2).reduce((a, b) => a + b, 0) / 2) * 0.4 +
  ...
  private calculateConfidence(
    ...
    return Math.min(0.95, Math.max(0.3, 
      baseConfidence * dataQuality * consistency
    ))
  }
  ...
  private generateEscalationTriggers(
    ...
    if (riskFactors.temporal.daysSinceLastSession > 21) {
      triggers.push('Extended absence from treatment - contact attempt required')
    }
  ...
  private calculateHourlyRisk(baseRisk: number, temporalFactors: CrisisRiskFactors['temporal']): number {
    // Adjust for time of day
    const timeMultiplier = (temporalFactors.timeOfDay >= 22 || temporalFactors.timeOfDay <= 6) ? 1.3 : 0.8
    return Math.min(1, baseRisk * timeMultiplier)
  }
}

export const predictiveCrisisModelingService = new PredictiveCrisisModelingService()