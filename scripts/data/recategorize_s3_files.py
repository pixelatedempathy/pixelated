...
from ai.infrastructure.s3.s3_dataset_loader import S3Config, S3DatasetLoader
...
class S3Recategorizer:
    ...
    def __init__(self, output_dir: Path = Path("metrics"), confidence_threshold: float = 0.70):
        """
    ...
    def list_files_to_recategorize(
        ...
        try:
            ...
            jsonl_files = [obj["Key"] for obj in all_keys if obj.get("Key", "").endswith(".jsonl")]
        ...
        try:
            ...
            for record in records:
                ...
                try:
                    ...
                    logger.error(f"Classification failed for record: {e}")
                    continue
            ...
            logger.info(
                f"✅ {filename}: {stats['classified']}/{stats['total_records']} "
    ...
    def process_all_files(
        ...
        # List files
        files_to_process = self.list_files_to_recategorize(prefix=input_prefix, limit=limit)
        ...
        stats.end_time = end_time_obj.isoformat()
        stats.duration_seconds = (end_time_obj - start_time_obj).total_seconds()
        ...
        with open(output_file, "w") as f:
            json.dump({"summary": stats.__dict__, "file_details": file_stats_list}, f, indent=2)
...
def main():
    ...
    logger.info(f"Classified: {stats.classified_records:,}")
    logger.info(f"Low confidence: {stats.low_confidence_records:,}")
