...
export class ExternalThreatFeedIntegration extends EventEmitter {
  ...
  private async processScheduledSyncs(): Promise<void> {
    try {
      const now = new Date()
      const feedsToSync = await this.feedsCollection
        .find({
          status: 'active',
          next_sync: { $lte: now },
        })
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to process scheduled syncs', {
        error: normalized.message,
      })
    }
  }
  ...
  async getThreatIndicators(
    ...
    try {
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to get threat indicators', {
        error: normalized.message,
      })
      throw error
    }
  ...
  async searchThreatIndicators(
    ...
    try {
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to search threat indicators', {
        error: normalized.message,
      })
      throw error
    }
  ...
  async getFeedById(feedId: string): Promise<ThreatFeed | null> {
    try {
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to get feed by ID', {
        error: normalized.message,
        feed_id: feedId,
      })
      throw error
  ...
  async getFeedSyncResults(
    ...
    try {
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to get feed sync results', {
        error: normalized.message,
        feed_id: feedId,
      })
      throw error
  ...
  async isKnownThreat(
    ...
    try {
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to check if value is known threat', {
        error: normalized.message,
        value,
        type,
      })
  ...
  async bulkCheckThreats(
    ...
    try {
      ...
      for (const item of values) {
        ...
        if (result.is_threat) {
          bySeverity[result.highest_severity] =
            (bySeverity[result.highest_severity] || 0) + 1
        }
      }
      ...
      const normalized = normalizeError(error)
      logger.error('Failed to bulk check threats', {
        error: normalized.message,
      })
      throw error
    }
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      if (this.activeSyncs.size > 0) {
        logger.info(
          `Waiting for ${this.activeSyncs.size} active syncs to complete`,
        )
        ...
        if (this.activeSyncs.size > 0) {
          logger.warn(
            `Force shutting down with ${this.activeSyncs.size} active syncs`,
          )
        }
      }
