...
export class DistributedRateLimiter {
  ...
  async checkLimit(
    ...
    try {
      ...
      logger.error('Rate limit check failed:', { error, identifier, rule: rule.name })
  ...
  private analyzePattern(timestamps: number[]): AttackPattern {
    ...
    if (variance < 1000) { // 1 second threshold
      return {
        isSuspicious: true,
  ...
  private async handleAttackPattern(
    ...
    await redis.setex(blockKey, 300, JSON.stringify({ // 5 minute block
      pattern,
      detectedAt: Date.now(),
      context
    }))
  ...
  async getStatus(identifier: string, rule: RateLimitRule): Promise<RateLimitResult> {
    ...
    try {
      ...
      logger.error('Failed to get rate limit status:', { error, identifier, rule: rule.name })
      return {
        allowed: true,
        limit: rule.maxRequests,
        remaining: rule.maxRequests,
