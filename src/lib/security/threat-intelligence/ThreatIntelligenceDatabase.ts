...
export class ThreatIntelligenceDatabase extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.stixCollection = this.db.collection<DatabaseThreatIntelligence>('stix_objects');
      this.collectionsCollection = this.db.collection<DatabaseCollection>('taxii_collections');
      this.manifestCollection = this.db.collection<TAXIIManifestEntry>('taxii_manifest');
  ...
  private async createIndexes(): Promise<void> {
    try {
      await Promise.all([
        ...
        this.stixCollection.createIndex({ expires_at: 1 }, { expireAfterSeconds: 0 }),

        // Collections indexes
        this.collectionsCollection.createIndex({ collection_id: 1 }, { unique: true }),
        this.collectionsCollection.createIndex({ title: 'text', description: 'text' }),
      ...
      for (const collection of this.config.taxii.collections) {
        ...
        if (!existing) {
          await this.collectionsCollection.insertOne({
            ...
            stix_types: ['indicator', 'malware', 'threat-actor', 'attack-pattern', 'vulnerability'],
          ...
          logger.info('TAXII collection created', { collection_id: collection.id });
        }
      }
    } catch (error) {
      logger.error('Failed to initialize TAXII collections', { error: error.message });
      throw error;
    }
  }
  ...
  async storeSTIXObject(
    ...
    try {
      ...
      if (this.config.encryption.enabled) {
        const encryptedData = await encrypt(JSON.stringify(stixObject), this.config.encryption.key);
        dataToStore = {
          ...dataToStore,
      ...
      const ttl = this.config.retention.stix_types[stixObject.type] || this.config.retention.default_ttl;
      ...
      this.emit('stix:stored', { stix_id: stixObject.id, type: stixObject.type });
  ...
  async getSTIXObject(stixId: string): Promise<STIXObject | null> {
    try {
      ...
      if (result.encrypted_data && this.config.encryption.enabled) {
        const decryptedData = await decrypt(result.encrypted_data, this.config.encryption.key);
        return JSON.parse(decryptedData);
      }

      return result.stix_object;
    } catch (error) {
      logger.error('Failed to get STIX object', { error: error.message, stix_id: stixId });
      throw error;
    }
  }
  ...
  async querySTIXObjects(query: {
    ...
    try {
      ...
      if (query.collection_id) filter['metadata.collection_id'] = query.collection_id;
      if (query.source) filter['metadata.source'] = query.source;
      if (query.validated !== undefined) filter['metadata.validated'] = query.validated;
      if (query.processed !== undefined) filter['metadata.processed'] = query.processed;
      ...
      if (query.start_date || query.end_date) {
        ...
        if (query.start_date) filter['stix_object.created'].$gte = query.start_date.toISOString();
        if (query.end_date) filter['stix_object.created'].$lte = query.end_date.toISOString();
      }
      ...
      const decryptedObjects = objects.map(obj => {
        if (obj.encrypted_data && this.config.encryption.enabled) {
          return JSON.parse(decrypt(obj.encrypted_data, this.config.encryption.key));
        }
      ...
      logger.error('Failed to query STIX objects', { error: error.message, query });
      throw error;
    }
  }
   ...
   */
  async getTAXIICollection(collectionId: string): Promise<TAXIICollection | null> {
    try {
      ...
      logger.error('Failed to get TAXII collection', { error: error.message, collectionId });
      throw error;
    }
  }
   ...
   */
  async getTAXIIManifest(collectionId: string, limit: number = 100): Promise<TAXIIManifestEntry[]> {
    try {
      ...
      logger.error('Failed to get TAXII manifest', { error: error.message, collectionId });
      throw error;
    }
  }
  ...
  async getTAXIIObjects(
    ...
    try {
      ...
      const limit = Math.min(options.limit || 100, this.config.taxii.max_page_size);
      ...
      logger.error('Failed to get TAXII objects', { error: error.message, collectionId });
      throw error;
    }
  }
   ...
   * Update TAXII manifest
   */
  private async updateTAXIIManifest(stixId: string, collectionId: string): Promise<void> {
    try {
      ...
      logger.error('Failed to update TAXII manifest', { error: error.message, stixId, collectionId });
      throw error;
    }
  }
  ...
  async validateSTIXObjectData(stixId: string): Promise<{
    ...
    try {
      ...
      if (obj.encrypted_data && this.config.encryption.enabled) {
        const decryptedData = await decrypt(obj.encrypted_data, this.config.encryption.key);
      ...
      if (stixObject.spec_version && stixObject.spec_version !== this.config.stix.spec_version) {
        warnings.push(`STIX spec version mismatch: ${stixObject.spec_version} vs ${this.config.stix.spec_version}`);
      }
      ...
      logger.error('Failed to validate STIX object', { error: error.message, stixId });
      throw error;
    }
  }
  ...
  async processSTIXObject(stixId: string): Promise<{
    ...
    try {
      ...
      switch (stixObject.type) {
        case 'indicator': {
          const indicatorResult = await this.processIndicator(stixObject as STIXIndicator);
          enrichments.push(...indicatorResult.enrichments);
          normalized = indicatorResult.normalized;
          break;
        }
        case 'malware': {
          const malwareResult = await this.processMalware(stixObject as STIXMalware);
      ...
      if (this.config.encryption.enabled && obj.encrypted_data) {
        const encryptedData = await encrypt(JSON.stringify(updatedData), this.config.encryption.key);
        await this.stixCollection.updateOne(
          { stix_id: stixId },
          { 
            $set: { 
      ...
      logger.error('Failed to process STIX object', { error: error.message, stixId });
      throw error;
    }
  }
  ...
  private async processIndicator(indicator: STIXIndicator): Promise<{
    ...
    try {
      ...
      if (indicator.kill_chain_phases) {
        indicator.kill_chain_phases = indicator.kill_chain_phases.filter(phase => 
  ...
  private isValidKillChainPhase(phase: KillChainPhase): boolean {
    return this.config.stix.kill_chain_phases.some(validPhase => 
  ...
  async getDatabaseStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        by_type: byTypeMap,
        collections,
        processed_percentage: totalObjects > 0 ? (processedCount / totalObjects) * 100 : 0,
        validated_percentage: totalObjects > 0 ? (validatedCount / totalObjects) * 100 : 0,
      };
    } catch (error) {
