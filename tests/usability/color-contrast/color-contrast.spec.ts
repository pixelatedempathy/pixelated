...
test.describe('Color Contrast Compliance', () => {
  ...
  testPages.forEach(({ path, name }) => {
    test(`${name} should meet WCAG AA color contrast requirements`, async ({
      page,
    }) => {
  ...
  test('should maintain contrast in dark mode', async ({ page }) => {
    ...
    const darkModeToggle = page.locator(
      '[data-testid="theme-toggle"], .dark-mode-toggle',
    )
  ...
  test('should maintain contrast with custom themes', async ({ page }) => {
    ...
    const themeSelectors = await page
      .locator('[data-testid*="theme"], .theme-selector option')
      .all()

    for (const selector of themeSelectors.slice(0, 3)) {
      // Test first 3 themes
