...
- **`bias_detection/`** - FastAPI-based service package
...
- The Flask service (`bias_detection_service.py`) is the primary production
  service
- The FastAPI service (`bias_detection/`) is experimental and not yet in
  production
- Both services share similar functionality but use different frameworks
- Consider migrating to FastAPI in the future for better async support and
