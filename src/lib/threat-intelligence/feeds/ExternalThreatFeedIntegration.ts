...
export interface ExternalThreatFeedIntegration {
  ...
  processFeedItems(subscriptionId: string, items: FeedItem[]): Promise<FeedProcessingResult>;
  getFeedStatus(subscriptionId: string): Promise<FeedStatus>;
  getAllSubscriptions(): Promise<FeedSubscription[]>;
  updateFeedConfig(subscriptionId: string, config: Partial<FeedConfig>): Promise<boolean>;
...
export class ExternalThreatFeedIntegrationCore extends EventEmitter implements ExternalThreatFeedIntegration {
  private redis: Redis;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error('Failed to initialize External Threat Feed Integration System:', { error });
  ...
  private async initializeMongoDB(): Promise<void> {
    try {
      this.mongoClient = new MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017/threat_feeds');
  ...
  private validateFeedConfig(feedConfig: FeedConfig): void {
    ...
    // Validate feed type
    const validFeedTypes = ['stix', 'taxii', 'misp', 'otx', 'virustotal', 'generic'];
    ...
    if (feedConfig.updateFrequency && !validFrequencies.includes(feedConfig.updateFrequency)) {
      throw new Error(`Invalid update frequency: ${feedConfig.updateFrequency}`);
    }
  }

  private async createSubscription(feedConfig: FeedConfig): Promise<FeedSubscription> {
  ...
  private async storeSubscription(subscription: FeedSubscription): Promise<void> {
    try {
      const subscriptionsCollection = this.db.collection('feed_subscriptions');
  ...
  private async startFeedProcessingForSubscription(subscription: FeedSubscription): Promise<void> {
    try {
      const interval = this.getFeedProcessingInterval(subscription.updateFrequency);
      ...
      logger.error('Failed to start feed processing for subscription:', { error });
    }
  }
  ...
  private async processFeedForSubscription(subscription: FeedSubscription): Promise<void> {
    try {
      ...
      // Process feed items
      const processingResult = await this.processFeedItems(subscription.subscriptionId, feedItems);
  ...
  private async fetchFeedItems(subscription: FeedSubscription): Promise<FeedItem[]> {
    try {
      const processor = this.getFeedProcessor(subscription.feedType);
      if (!processor) {
        throw new Error(`No processor found for feed type: ${subscription.feedType}`);
      }
      ...
      const filteredItems = this.filterFeedItems(feedItems, subscription.filters);
      
      // Deduplicate items
      const deduplicatedItems = await this.deduplicateFeedItems(filteredItems, subscription);
  ...
  private async buildFeedRequestConfig(subscription: FeedSubscription): Promise<any> {
    const config: any = {
  ...
  private filterFeedItems(items: FeedItem[], filters: Record<string, any>): FeedItem[] {
  ...
  private async deduplicateFeedItems(items: FeedItem[], subscription: FeedSubscription): Promise<FeedItem[]> {
    try {
  ...
  async processFeedItems(subscriptionId: string, items: FeedItem[]): Promise<FeedProcessingResult> {
    try {
      ...
      for (let i = 0; i < items.length; i += batchSize) {
        ...
        try {
          const batchResult = await this.processFeedBatch(batch, processor, subscription);
  ...
  private async storeProcessedThreats(threats: GlobalThreatIntelligence[], subscription: FeedSubscription): Promise<void> {
    try {
  ...
  private async updateSubscription(subscription: FeedSubscription): Promise<void> {
    try {
      const subscriptionsCollection = this.db.collection('feed_subscriptions');
  ...
  async unsubscribeFromFeed(subscriptionId: string): Promise<boolean> {
    try {
      ...
      logger.error('Failed to unsubscribe from feed:', { error, subscriptionId });
      return false;
    }
  }
  ...
  async updateFeedConfig(subscriptionId: string, config: Partial<FeedConfig>): Promise<boolean> {
    try {
      ...
      logger.error('Failed to update feed configuration:', { error, subscriptionId });
      return false;
    }
  }
  ...
  private async calculateTotalItemsProcessed(): Promise<number> {
    try {
      const subscriptionsCollection = this.db.collection('feed_subscriptions');
      const result = await subscriptionsCollection.aggregate([
  ...
  private async calculateAverageProcessingTime(): Promise<number> {
    try {
      const processingLogsCollection = this.db.collection('feed_processing_logs');
      const result = await processingLogsCollection.aggregate([
  ...
  private async getFeedsByType(): Promise<Record<string, number>> {
    try {
      ...
      const results = await subscriptionsCollection.aggregate(pipeline).toArray();
  ...
  async getHealthStatus(): Promise<HealthStatus> {
    try {
      ...
      const metrics = await this.getFeedMetrics();
      const successRate = metrics.totalSubscriptions > 0 ? 
        (metrics.activeSubscriptions / metrics.totalSubscriptions) * 100 : 0;
...
export interface FeedProcessor {
  ...
  convertToThreat(item: FeedItem, subscription: FeedSubscription): Promise<GlobalThreatIntelligence | null>;
}
...
class STIXFeedProcessor implements FeedProcessor {
  async parseFeed(data: any, subscription: FeedSubscription): Promise<FeedItem[]> {
    try {
  ...
  async convertToThreat(item: FeedItem, subscription: FeedSubscription): Promise<GlobalThreatIntelligence | null> {
    try {
...
class TAXIIFeedProcessor implements FeedProcessor {
  ...
  private extractIndicatorFromTAXII(obj: any): string {
    ...
    if (obj.external_references && obj.external_references.length > 0) {
      return obj.external_references[0].url || obj.external_references[0].external_id;
    }
    return obj.id;
  }
  ...
  private mapTAXIIThreatLevel(obj: any): string {
    ...
    if (obj.labels && obj.labels.includes('suspicious-activity')) return 'medium';
    return 'low';
  }
...
class MISPFeedProcessor implements FeedProcessor {
  ...
  private mapMISPSeverity(comment: string): string {
    if (comment.includes('critical') || comment.includes('high')) return 'critical';
      ...
      if (data.data) {
        for (const file of data.data) {
          if (file.attributes && file.attributes.last_analysis_stats) {
            ...
            const totalCount = stats.malicious + stats.suspicious + stats.undetected + stats.harmless;
            ...
            if (maliciousCount > 0) {
              items.push({
                ...
                description: file.attributes.meaningful_name || 'Malicious file',
                source: subscription.provider,
                metadata: {
