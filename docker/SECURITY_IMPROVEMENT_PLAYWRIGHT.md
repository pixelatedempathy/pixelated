...
- **Eliminated Full Host Access**: Container no longer has unrestricted access
  to host resources
- **Maintained Functionality**: Chromium browser continues to work properly with
  sandboxing
- **Principle of Least Privilege**: Container only gets the specific system
  calls it needs
...
- Specifically allows targeted rules for capabilities like CAP_SYS_ADMIN,
  CAP_SYS_PTRACE, etc.
...
`docker compose -f docker-compose.playwright.yml config` and shows proper
parsing of the security options.

This change resolves the Codacy security alert while maintaining all Playwright
