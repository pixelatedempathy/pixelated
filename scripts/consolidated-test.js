...
function main() {
  ...
  if (testType === 'all') {
    ...
    log(
      'Available options: all, typescript, lint, format, vitest, python, e2e, integration',
      'yellow',
    )
    process.exit(1)
  }
  ...
  for (const suite of suitesToRun) {
    ...
    if (success) {
      ...
      log(
        `\n${colors.bold}❌ Required test failed: ${config.description}${colors.reset}`,
        'red',
      )
      if (testType === 'all') {
  ...
  if (requiredFailures > 0) {
    log(
      `\n${colors.bold}❌ ${requiredFailures} required test(s) failed${colors.reset}`,
      'red',
    )
    process.exit(1)
  } else {
