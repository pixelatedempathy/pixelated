...
export const TherapistDashboard: FC = () => {
  ...
  const [dashboardView, setDashboardView] = usePersistentState<'overview' | 'patients' | 'analytics' | 'schedule'>('therapist_dashboard_view', 'overview')
  const [timeRange, setTimeRange] = usePersistentState<'week' | 'month' | 'quarter' | 'year'>('therapist_dashboard_timerange', 'month')
  const [selectedPatients, setSelectedPatients] = usePersistentState<string[]>('therapist_selected_patients', [])
  ...
  const analyticsData = patients.map((patient, index) => ({
    ...
    riskScore: patient.riskLevel === 'critical' ? 4 : patient.riskLevel === 'high' ? 3 : patient.riskLevel === 'medium' ? 2 : 1,
...
const OverviewTab: FC<{
  ...
  const urgentPatients = patients.filter(p => p.riskLevel === 'high' || p.riskLevel === 'critical')
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white">{metrics.totalSessions}</p>
              </div>
              <div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
                <span className="text-blue-600 dark:text-blue-400">📊</span>
              </div>
            </div>
            <p className="text-sm text-gray-500 mt-2">This {timeRange === 'week' ? 'week' : timeRange === 'month' ? 'month' : timeRange}</p>
          </div>
        </FadeIn>
        ...
        <FadeIn>
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Avg Session Length</p>
                <p className="text-3xl font-bold text-gray-900 dark:text-white">{metrics.avgSessionLength}m</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900 dark:text-white">{metrics.patientSatisfaction}/5</p>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="space-y-3">
              {urgentPatients.map((patient) => (
                <div key={patient.id} className="flex items-center justify-between p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="space-y-3">
              {patientsNeedingAttention.map((patient) => (
                <div key={patient.id} className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-4">
            {patients.map((patient) => (
              <div key={patient.id} className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <input
                  type="checkbox"
                  checked={selectedPatients.includes(patient.id)}
                  onChange={() => onPatientSelect(patient.id)}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600 dark:text-gray-400">
                    <div>Last session: {patient.lastSession.toLocaleDateString()}</div>
                    <div>Progress: {patient.progress}%</div>
                    <div>Next appointment: {patient.nextAppointment?.toLocaleDateString() || 'Not scheduled'}</div>
...
const AnalyticsTab: FC<{
  ...
  const visualizationConfig = {
    ...
    dimensions: {
      x: { field: 'sessionsCompleted', label: 'Sessions Completed', type: 'numeric' as const },
      y: { field: 'avgMoodScore', label: 'Average Mood Score', type: 'numeric' as const },
      color: { field: 'riskScore', label: 'Risk Level', type: 'numeric' as const },
    },
    filters: {},
    interactive: true,
    realTime: false,
...
const ScheduleTab: FC<{
  ...
  const upcomingAppointments = patients
    ...
    .sort((a, b) => (a.nextAppointment?.getTime() || 0) - (b.nextAppointment?.getTime() || 0))
    .slice(0, 10)
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {upcomingAppointments.map((patient) => (
              <div key={patient.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900 dark:text-white">{patient.name}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {patient.nextAppointment?.toLocaleDateString()} at {patient.nextAppointment?.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(patient.riskLevel)}`}>
                  {patient.riskLevel}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3">
            {[
              ...
              >
                <p className="font-medium text-blue-900 dark:text-blue-100">{template}</p>
                <p className="text-sm text-blue-700 dark:text-blue-200">Standard 50-minute session</p>
              </button>
            ))}
          </div>
        </div>
