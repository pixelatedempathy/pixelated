...
describe("useTherapistAnalytics", () => {
  ...
  it("initializes with loading state", async () => {
    const { result } = renderHook(() => useTherapistAnalytics(mockFilters, mockSessions));
  ...
  it("transforms skill progress data correctly", async () => {
    ...
    const activeListening = skillProgress?.find(s => s.skill === 'Active Listening');
  ...
  it("generates summary stats correctly", async () => {
    ...
    const totalSessions = summaryStats?.find(s => s.label === 'Total Sessions');
  ...
  it("handles empty sessions array", async () => {
    const { result } = renderHook(() =>
      useTherapistAnalytics(mockFilters, [])
    );
  ...
  it("handles single session for comparative data", async () => {
    const singleSession = [mockSessions[0]].filter(Boolean) as TherapistSession[];
    const { result } = renderHook(() =>
