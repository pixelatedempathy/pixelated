...
const FHEDemo: React.FC<FHEDemoProps> = ({
  ...
  const [currentOperation, setCurrentOperation] = useState<Partial<FHEOperation>>({
    operation: 'add',
    input1: 15,
    input2: 25,
  })
  ...
  const performFHEOperation = useCallback(async (operation: FHEOperation): Promise<FHEOperation> => {
    ...
    return {
      ...
      encryptedInput2: operation.input2 ? simulateEncryption(operation.input2) : undefined,
  ...
  const runBenchmark = async (operation: FHEOperation) => {
    ...
    switch (operation.operation) {
      ...
      case 'compare':
        ...
        break
      case 'aggregate':
        plaintextResult = Math.round((operation.input1 + (operation.input2 || 0)) / 2)
        break
      default:
        plaintextResult = 0
    }
  ...
  if (isLoading) {
    return (
      <div className={`flex flex-col items-center justify-center h-64 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-lg ${className}`}>
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
        <h3 className="text-lg font-semibold text-gray-800 mb-2">Initializing FHE Library</h3>
        <p className="text-gray-600 text-center max-w-md">
          Loading Microsoft SEAL WebAssembly module for Fully Homomorphic Encryption operations...
        </p>
      </div>
    )
  }
    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          ...
          This demo simulates FHE operations for privacy-preserving therapy data analysis.
        </p>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        ...
        <div className="text-sm text-gray-600">
          <p>
            <strong>Privacy Note:</strong> All computations are performed on encrypted data. 
            The raw values are never exposed during processing.
          </p>
        </div>
      </div>
                ...
                <div key={operation.id} className="border rounded-lg p-4 bg-white shadow-sm">
                  ...
                  <div className="text-sm space-y-1">
                    <div>
                      ...
                      {operation.input2 !== undefined && ` ${operation.operation === 'add' ? '+' : operation.operation === 'multiply' ? '×' : 'vs'} ${operation.input2}`}
                    </div>
                    ...
                    {showAdvanced && operation.encryptedInput1 && (
                      <div className="mt-2 text-xs">
                        <div className="font-medium text-gray-700">Encrypted Data:</div>
                  ...
                  <div key={index} className="border rounded-lg p-4 bg-white shadow-sm">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>Plaintext:</span>
                        <span className="font-mono">{result.plaintextTime.toFixed(3)}ms</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>FHE:</span>
                        <span className="font-mono">{result.fheTime.toFixed(2)}ms</span>
                      </div>
                      ...
                      <div className="mt-2">
                        <div className="bg-gray-200 h-2 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${Math.min(100, (result.plaintextTime / result.fheTime) * 100)}%` }}
                          />
                        </div>
      ...
      <div className="mt-6 bg-blue-50 rounded-lg p-4">
        <h4 className="font-semibold text-blue-900 mb-2">💡 Why FHE Matters for Therapy Data</h4>
        <ul className="text-sm text-blue-800 space-y-1">
          <li>• <strong>Privacy-Preserving Analytics:</strong> Analyze patient data without seeing raw values</li>
          <li>• <strong>Secure Multi-Party Computation:</strong> Multiple therapists can collaborate on insights</li>
          <li>• <strong>Regulatory Compliance:</strong> HIPAA-compliant processing of sensitive health data</li>
          <li>• <strong>Research Applications:</strong> Enable large-scale studies while protecting individual privacy</li>
        </ul>
      </div>
    </div>
  )
