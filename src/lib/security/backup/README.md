...
This module provides interfaces and implementations for storage providers used
by the backup security system to store and retrieve encrypted backups.
...
6. **Mock Cloud Storage** - Simulates cloud storage behavior with local files
   (for development)

## Cloud Provider Dependencies

The cloud storage providers use dynamic imports to avoid requiring their
dependencies for all users. You need to install the appropriate packages
depending on which storage provider you plan to use:
...
Import the `getStorageProvider` function to create an instance of a storage
provider:
