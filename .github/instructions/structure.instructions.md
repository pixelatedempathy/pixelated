...
- `components/` - Domain-organized React components (admin/, ai/, auth/, chat/,
  dashboard/, ui/)
- `pages/` - Astro file-based routing with API routes in `pages/api/`
- `layouts/` - Page layout components
- `lib/` - Shared utilities organized by domain (ai/, auth/, db/, security/,
  utils/)
...
- **Components**: PascalCase with descriptive names
  (`TherapeuticSessionManager`)
- **Functions**: camelCase, verb-based (`validateTherapeuticInput`,
  `processEmotionalData`)
- **Constants**: UPPER_SNAKE_CASE (`MAX_SESSION_DURATION`, `BIAS_THRESHOLD`)
- **Types/Interfaces**: PascalCase with suffixes (`TherapeuticSessionData`,
  `BiasDetectionResult`)
