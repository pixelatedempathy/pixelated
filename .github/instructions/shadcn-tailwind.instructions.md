...
className =
  'flex items-center gap-4 w-full h-12 text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 focus:ring-2'
```
...
- **Local state**: `useState` for simple values, `useReducer` for complex
  objects
...
- **Tree-shake Tailwind**: Configure `content` paths precisely in
  `tailwind.config.ts`
- **Optimize re-renders**: Use `memo()` for expensive components,
...
describe('BiasAlert', () => {
  it('displays bias warning with correct severity', () => {
    render(
      <BiasAlert severity='high' message='Potential gender bias detected' />,
    )

    expect(screen.getByRole('alert')).toBeInTheDocument()
    expect(
      screen.getByText('Potential gender bias detected'),
    ).toBeInTheDocument()
