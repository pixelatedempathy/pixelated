...
async def get_connections(
    ...
    try:
        ...
        logger.error("Error getting connections", error=str(e), exc_info=True)
...
async def get_websocket_stats(
    ...
    try:
        ...
        authenticated_connections = len(
            [c for c in connections if c.status.value == "authenticated"]
        )
        ...
        logger.error("Error getting WebSocket stats", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def broadcast_message(
    ...
    try:
        ...
        if user_role not in ["admin", "system"]:
            raise HTTPException(
                status_code=403, detail="Insufficient permissions to broadcast messages"
            )
        ...
        logger.error("Error broadcasting message", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def update_task_progress(
    ...
    try:
        ...
        await websocket_manager.publish_event(
            "task:progress", progress_data, room=f"task:{task_id}"
        )
        ...
        logger.error("Error updating task progress", task_id=task_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def update_agent_status(
    ...
    try:
        ...
        await websocket_manager.publish_event(
            "agent:status_updated", status_data, room=f"agent:{agent_id}"
        )
        ...
        logger.error("Error updating agent status", agent_id=agent_id, error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def broadcast_coordination_message(
    ...
    try:
        ...
        if user_role not in ["admin", "agent", "system"]:
            raise HTTPException(
                status_code=403, detail="Insufficient permissions for coordination messages"
            )
        ...
        await websocket_manager.publish_event(
            "coordination:message", coord_data, room=f"coordination:{message.room}"
        )
        ...
        logger.error("Error broadcasting coordination message", error=str(e), exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def get_websocket_health(
    ...
    try:
        ...
        logger.error("Error getting WebSocket health", error=str(e), exc_info=True)
        return {
            "status": "unhealthy",
            "service": "websocket",
