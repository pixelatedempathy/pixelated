...
export class PromptOptimizerService {
   ...
   */
  async createConversationSummary(context: ConversationContext): Promise<ConversationSummary> {
    try {
      logger.info('Creating conversation summary', { sessionId: context.sessionId })

      // Check cache first
      if (this.summaryCache.has(context.sessionId)) {
        const cached = this.summaryCache.get(context.sessionId)!
        logger.debug('Returning cached summary', { sessionId: context.sessionId })
        return cached
      }
      ...
      const interventionAnalysis = this.analyzeInterventions(context.interventions)
      const insights = this.generateInsights(context)
      const recommendations = this.generateRecommendations(context, emotionalJourney)
  ...
  async optimizePrompt(
    ...
    try {
      ...
      const optimizedPrompt = this.applyOptimization(prompt, optimizationStrategy, context)
      const reasoning = this.generateOptimizationReasoning(prompt, optimizedPrompt, context)
      const confidenceScore = this.calculateOptimizationConfidence(context, reasoning)
   ...
   */
  private extractKeyTopics(messages: ConversationContext['messages']): string[] {
    ...
    const topicPatterns = {
      ...
      'self-esteem': ['confidence', 'self-worth', 'doubt', 'insecure', 'worthless'],
   ...
   */
  private analyzeEmotionalJourney(messages: ConversationContext['messages']): ConversationSummary['emotionalJourney'] {
    ...
    for (let i = 1; i < emotionalMessages.length; i++) {
      ...
      if (Math.abs(valenceChange) > 0.3) {
        ...
        keyTransitions.push({
          ...
          from,
          to,
          trigger: this.identifyTransitionTrigger(emotionalMessages[i - 1], emotionalMessages[i])
        })
      }
    }
  ...
  private identifyTransitionTrigger(
    ...
    const triggers = {
      'therapeutic_technique': ['breathing', 'mindfulness', 'reframe', 'technique'],
   ...
   */
  private analyzeInterventions(interventions: ConversationContext['interventions']): ConversationSummary['interventions'] {
   ...
   */
  private estimateInterventionEffectiveness(intervention: ConversationContext['interventions'][0]): number {
    ...
    const positiveIndicators = ['better', 'helpful', 'improved', 'clearer', 'good', 'worked']
    const negativeIndicators = ['worse', 'difficult', 'unhelpful', 'confused', 'bad', 'failed']
  ...
  private generateInsights(context: ConversationContext): string[] {
    ...
    const avgMessageLength = patientMessages.reduce((sum, m) => sum + m.content.length, 0) / patientMessages.length
    
    if (avgMessageLength > 200) {
      insights.push('Patient demonstrates good verbal engagement with detailed responses')
    } else if (avgMessageLength < 50) {
      insights.push('Patient responses are brief - may indicate discomfort or disengagement')
    }
    ...
    if (interventionTypes.size > 3) {
      insights.push('Multiple intervention approaches used - suggests adaptive therapeutic style')
    }

    // Analyze session duration patterns
    if (context.sessionMetadata.duration > 60) {
      insights.push('Extended session duration may indicate complex issues or high engagement')
    }
  ...
  private generateRecommendations(
    ...
    const valenceChange = emotionalJourney.final.dimensions.valence - emotionalJourney.initial.dimensions.valence
    
    if (valenceChange < -0.2) {
      recommendations.push('Consider crisis assessment - emotional state declined during session')
      recommendations.push('Schedule follow-up within 24-48 hours')
    } else if (valenceChange > 0.3) {
      recommendations.push('Continue current therapeutic approach - positive emotional progress observed')
    }

    // Engagement recommendations
    const patientMessages = context.messages.filter(m => m.role === 'patient')
    if (patientMessages.length < 5) {
      recommendations.push('Focus on increasing patient engagement in future sessions')
    }

    // Intervention recommendations
    const effectiveInterventions = context.interventions.filter(i => 
      i.outcome && i.outcome.toLowerCase().includes('helpful')
    )
    
    if (effectiveInterventions.length > 0) {
      recommendations.push(`Continue using ${effectiveInterventions[0].type} interventions - showed positive results`)
    }
  ...
  private identifyRiskFactors(
    ...
    // High-risk keywords
    const riskKeywords = ['suicide', 'self-harm', 'hurt myself', 'end it all', 'no point', 'give up']
    const conversationText = context.messages.map(m => m.content).join(' ').toLowerCase()
    ...
    const highNegativeEmotions = finalEmotion.emotions.sadness + finalEmotion.emotions.anger + finalEmotion.emotions.fear
   ...
   */
  private calculateProgressMetrics(context: ConversationContext): ConversationSummary['progressMetrics'] {
    const patientMessages = context.messages.filter(m => m.role === 'patient')
    ...
    if (emotionalMessages.length >= 2) {
      const initial = emotionalMessages[0].emotions!
      const final = emotionalMessages[emotionalMessages.length - 1].emotions!
      overallImprovement = (final.dimensions.valence - initial.dimensions.valence + 1) / 2
    }

    // Engagement level based on message frequency and length
    const engagementLevel = Math.min(1, patientMessages.length * 0.1) * 
                           Math.min(1, patientMessages.reduce((sum, m) => sum + m.content.length, 0) / 1000)

    // Stability based on emotional variance
    let stabilityScore = 0.5
    if (emotionalMessages.length > 2) {
      const valences = emotionalMessages.map(m => m.emotions!.dimensions.valence)
  ...
  private generateNarrativeSummary(
    ...
    const topicsList = keyTopics.length > 0 ? keyTopics.join(', ') : 'general discussion'
    
    const valenceChange = emotionalJourney.final.dimensions.valence - emotionalJourney.initial.dimensions.valence
    const emotionalDirection = valenceChange > 0.1 ? 'improved' : valenceChange < -0.1 ? 'declined' : 'remained stable'
    
    const effectiveInterventions = interventions.filter(i => i.effectiveness > 0.6)
    const interventionSummary = effectiveInterventions.length > 0 
      ? `Effective interventions included ${effectiveInterventions.map(i => i.type).join(', ')}.`
      : 'Interventions showed mixed results.'

    return `${sessionDuration}-minute session focusing on ${topicsList}. ` +
   ...
   */
  private selectOptimizationStrategy(context: PromptOptimization['therapeuticContext']): string {
  ...
  private addStabilizationElements(prompt: string): string {
    ...
    const randomPhrase = stabilizingPhrases[Math.floor(Math.random() * stabilizingPhrases.length)]
  ...
  private addRapportElements(prompt: string): string {
    ...
    const randomPhrase = rapportPhrases[Math.floor(Math.random() * rapportPhrases.length)]
    return `${randomPhrase}. ${prompt}`
  }
  ...
  private emphasizeTechniques(prompt: string, primaryConcern: string): string {
    ...
    const techniquePrompt = techniqueMap[primaryConcern] || 'What strategies have worked for you before?'
  ...
  private addConsolidationElements(prompt: string): string {
    ...
    const randomPhrase = consolidationPhrases[Math.floor(Math.random() * consolidationPhrases.length)]
  ...
  private generateOptimizationReasoning(
    ...
    if (optimized.length > original.length) {
      reasoning.push('Added supportive language to increase therapeutic rapport')
    }
    
    if (context.patientState === 'distressed') {
      reasoning.push('Incorporated stabilization techniques for distressed patient state')
    }
    ...
    if (optimized.includes('take your time') || optimized.includes('no pressure')) {
      reasoning.push('Reduced pressure and time constraints to improve comfort')
    }
    
    return reasoning.length > 0 ? reasoning : ['Applied general therapeutic communication principles']
  }
