...
export class BiasDetectionEngine {
  ...
  async analyzeSession(session: SessionData): Promise<AnalysisResult> {
    ...
    // Parallelize independent analysis layers to reduce latency from ~800ms to ~200ms
    // Use Promise.all with individual error handling to maintain fallback logic
    const results = await Promise.all([
      this.getPreprocessingResult(session),
      this.getModelLevelResult(session),
      this.getInteractiveResult(session),
      this.getEvaluationResult(session)
    ])

    const [preprocessingResult, modelLevelResult, interactiveResult, evaluationResult] = results

    // Maintain deterministic order for recommendations
    if (preprocessingResult.error) recs.push(preprocessingResult.error)
    if (modelLevelResult.error) recs.push(modelLevelResult.error)
    if (interactiveResult.error) recs.push(interactiveResult.error)
    if (evaluationResult.error) recs.push(evaluationResult.error)

    const layerResults: LayerResults = {
      preprocessing: preprocessingResult.result,
      modelLevel: modelLevelResult.result,
      interactive: interactiveResult.result,
   ...
   */
  /**
   * Internal helpers for parallel layer analysis
   */
  private async getPreprocessingResult(session: SessionData): Promise<{
    result: import('./types').PreprocessingAnalysisResult
    error?: string
  }> {
    try {
      const result = await this.pythonService.runPreprocessingAnalysis(session)
      return { result }
    } catch {
      const fb = this.fallbackLayer()
      return {
        error: 'Preprocessing analysis unavailable; using fallback results',
        result: {
          ...
          recommendations: [],
        } as import('./types').PreprocessingAnalysisResult,
      }
    }
  }

  private async getModelLevelResult(session: SessionData): Promise<{
    result: import('./types').ModelLevelAnalysisResult
    error?: string
  }> {
    try {
      const result = await this.pythonService.runModelLevelAnalysis(session)
      return { result }
    } catch {
      const fb = this.fallbackLayer()
      return {
        error: 'Model-level analysis unavailable; using fallback results',
        result: {
          biasScore: fb.biasScore,
          fairnessMetrics: {
            demographicParity: 0,
          ...
          recommendations: [],
        } as import('./types').ModelLevelAnalysisResult,
      }
    }
  }

  private async getInteractiveResult(session: SessionData): Promise<{
    result: import('./types').InteractiveAnalysisResult
    error?: string
  }> {
    try {
      const result = await this.pythonService.runInteractiveAnalysis(session)
      return { result }
    } catch {
      const fb = this.fallbackLayer()
      return {
        error: 'Interactive analysis unavailable; using fallback results',
        result: {
          biasScore: fb.biasScore,
          ...
          whatIfScenarios: [],
          recommendations: [],
        } as import('./types').InteractiveAnalysisResult,
      }
    }
  }

  private async getEvaluationResult(session: SessionData): Promise<{
    result: import('./types').EvaluationAnalysisResult
    error?: string
  }> {
    try {
      const result = await this.pythonService.runEvaluationAnalysis(session)
      return { result }
    } catch {
      const fb = this.fallbackLayer()
      return {
        error: 'Evaluation analysis unavailable; using fallback results',
        result: {
          biasScore: fb.biasScore,
          huggingFaceMetrics: {
            toxicity: 0.05,
          ...
          recommendations: [],
        } as import('./types').EvaluationAnalysisResult,
      }
    }
  }
