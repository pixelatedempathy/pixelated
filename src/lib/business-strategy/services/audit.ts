...
export class AuditService {
  ...
  constructor() {
    ...
    this.auditCollection = db.mongodb.database.collection(
      db.mongodb.collections.auditLogs,
    )
  }
  ...
  async getEntityAuditLogs(
    ...
    const cursor = this.auditCollection.find(filter).sort({ timestamp: -1 })
   ...
   */
  async cleanupOldLogs(retentionDays: number = 2190): Promise<number> {
    // 6 years default
