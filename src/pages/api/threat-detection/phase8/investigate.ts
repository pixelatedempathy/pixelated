...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Start investigation
    const investigation =
      await threatDetectionSystem.huntingService.startInvestigation({
        threatId: sanitizedData.threatId,
        userId: sanitizedData.userId,
        severity: sanitizedData.severity as
          | 'low'
          | 'medium'
          | 'high'
          | 'critical',
...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    const result =
      await threatDetectionSystem.huntingService.getInvestigationResult(
