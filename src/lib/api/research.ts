...
export class ResearchAPI {
  ...
  async searchLiterature(params: SearchParams): Promise<SearchResponse> {
    ...
    if (params.min_relevance)
      queryParams.append('min_relevance', params.min_relevance.toString())
    if (params.year_from)
  ...
  async searchDatasets(params: DatasetParams): Promise<DatasetResponse> {
    ...
    queryParams.append('q', params.q)
    if (params.min_turns)
      queryParams.append('min_turns', params.min_turns.toString())
    if (params.min_quality)
      queryParams.append('min_quality', params.min_quality.toString())
    if (params.limit) queryParams.append('limit', params.limit.toString())

    const response = await this.fetchWithTimeout(
      `${this.baseURL}/datasets?${queryParams.toString()}`,
    )
