...
async def execute_pipeline(
    ...
    try:
        ...
        # Execute pipeline
        execution_id = await flask_service.execute_pipeline(request.pipeline_config, user_id)
        ...
        logger.warning(
            "Pipeline validation failed", user_id=current_user.get("user_id"), error=str(e)
        )
...
async def cancel_pipeline(
    ...
    try:
        ...
        raise HTTPException(status_code=403, detail="Cancellation not permitted.")
    except Exception as e:
...
async def update_pipeline_progress(
    ...
    try:
        ...
        if user_role not in ["system", "agent", "admin"]:
            raise HTTPException(
                status_code=403, detail="Insufficient permissions to update pipeline progress"
            )
