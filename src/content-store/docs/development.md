...
proceeding. <br /> Please upgrade to `docs.json` before proceeding and delete
the legacy `mint.json` file.
...
**Step 2**: Navigate to the docs directory (where the `docs.json` file is
...
by using the `--port` flag. To run Mintlify on port 3333, for instance, use this
command:
...
Mintlify. If your local website doesn't align with the production version,
...
The CLI can assist with validating reference links made in your documentation.
