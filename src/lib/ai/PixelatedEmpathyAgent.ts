...
export class PixelatedEmpathyAgent {
  ...
  async generateScenario(request: {
    condition: 'depression' | 'anxiety' | 'ptsd' | 'bipolar' | 'substance_use' | 'personality_disorder' | 'crisis';
...
export const createPixelatedEmpathyAgent = () => {
  ...
  if (!config.agentEndpoint || !config.apiKey || !config.agentId) {
    throw new Error('Missing Azure AI Agent configuration. Please set AZURE_AI_AGENT_ENDPOINT, AZURE_AI_AGENT_KEY, and AZURE_AI_AGENT_ID environment variables.');
  }
