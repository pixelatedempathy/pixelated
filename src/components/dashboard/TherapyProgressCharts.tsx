...
const SkillDevelopmentRadar: React.FC<{ skills?: TherapistAnalyticsChartData['skillProgress'] }> = ({ skills = [] }) => {
  if (!skills || skills.length === 0) return <div className="bg-muted rounded-md p-4 text-center text-muted-foreground">No skill data available</div>

  const top = [...skills].sort((a, b) => (b.score ?? 0) - (a.score ?? 0)).slice(0, 6)
      ...
      <div className="relative w-64 h-64 mx-auto">
        <div className="absolute inset-0">
          {[0, 25, 50, 75, 100].map((level) => (
            <div key={level} className="absolute border border-gray-200 rounded-full" style={{ width: `${level}%`, height: `${level}%`, top: `${(100 - level) / 2}%`, left: `${(100 - level) / 2}%` }} />
          ))}
        </div>
        ...
        {top.map((t, idx) => {
          ...
          return <div key={t.skillId} className="absolute w-2 h-2 bg-blue-500 rounded-full" style={{ left: `${x}%`, top: `${y}%` }} title={`${t.skill}: ${t.score ?? 0}%`} />
        })}
        ...
        {top.map((t, idx) => {
          ...
          return <div key={`label-${t.skillId}`} className="absolute text-xs text-gray-600 transform -translate-x-1/2 -translate-y-1/2" style={{ left: `${x}%`, top: `${y}%` }}>{t.skill}</div>
        })}
      </div>
    </div>
  )
...
const SessionComparison: React.FC<{ comparativeData?: TherapistAnalyticsChartData['comparativeData'] }> = ({ comparativeData }) => {
  ...
  const items = [
    ...
    { label: 'Response Time', current: currentSession.averageResponseTime ?? 0, previous: previousSession.averageResponseTime ?? 0, unit: 's' },
    { label: 'Milestones', current: currentSession.milestonesAchieved ?? 0, previous: previousSession.milestonesAchieved ?? 0, unit: '' }
  ]
    ...
    <div className="bg-white p-4 rounded-lg shadow">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm">
          ...
          <div className="text-muted-foreground">{String(currentSession.sessionId).slice(0, 8)}</div>
        </div>
        <div className={cn('px-3 py-1 rounded-full text-sm font-medium', trend === 'improving' ? 'bg-green-100 text-green-800' : trend === 'declining' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800')}>
          {trend === 'improving' ? '↗ Improving' : trend === 'declining' ? '↘ Declining' : '→ Stable'}
        </div>
        <div className="text-sm text-right">
          <div className="font-medium">Previous Session</div>
          <div className="text-muted-foreground">{String(previousSession.sessionId).slice(0, 8)}</div>
        </div>
      </div>
          ...
          return (
            <div key={idx} className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium">{it.current}{it.unit}</span>
                <span className={cn('text-xs px-2 py-1 rounded', diff > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800')}>{diff > 0 ? '+' : ''}{diff.toFixed(1)}{it.unit}</span>
                <span className="text-sm text-muted-foreground">{it.previous}{it.unit}</span>
              </div>
            </div>
          )
        })}
...
const SkillImprovementTimeline: React.FC<{ skills?: TherapistAnalyticsChartData['skillProgress'] }> = ({ skills = [] }) => {
  if (!skills || skills.length === 0) return <div className="bg-muted rounded-md p-4 text-center text-muted-foreground">No skill improvement data available</div>
  const sorted = [...skills].sort((a, b) => (b.sessionsPracticed ?? 0) - (a.sessionsPracticed ?? 0))
          ...
          <div key={skill.skillId} className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{skill.score ?? 0}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
                <div className={cn('h-2 rounded-full transition-all duration-500', skill.trend === 'up' ? 'bg-green-500' : skill.trend === 'down' ? 'bg-red-500' : 'bg-blue-500')} style={{ width: `${skill.score ?? 0}%` }} />
              </div>
              <div className="flex items-center justify-between mt-1">
                <span className="text-xs text-muted-foreground">{skill.sessionsPracticed ?? 0} sessions</span>
                <span className={cn('text-xs', skill.trend === 'up' ? 'text-green-600' : skill.trend === 'down' ? 'text-red-600' : 'text-gray-600')}>{skill.trend === 'up' ? '↗' : skill.trend === 'down' ? '↘' : '→'}</span>
              </div>
            </div>
          </div>
        ))}
