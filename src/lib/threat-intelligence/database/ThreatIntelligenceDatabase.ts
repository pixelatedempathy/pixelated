...
export interface ThreatIntelligenceDatabase {
  ...
  getThreatByIntelligenceId(intelligenceId: string): Promise<GlobalThreatIntelligence | null>;
  getThreatByIndicator(indicatorType: string, indicatorValue: string): Promise<GlobalThreatIntelligence | null>;
  ...
  getRecentThreats(region?: string, limit?: number): Promise<GlobalThreatIntelligence[]>;
  ...
  searchThreats(query: any, pagination: PaginationParams): Promise<ApiResponse<GlobalThreatIntelligence[]>>;
...
export class ThreatIntelligenceDatabaseCore extends EventEmitter implements ThreatIntelligenceDatabase {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.error('Failed to initialize Threat Intelligence Database:', { error });
  ...
  private async initializeRedis(): Promise<void> {
    try {
      ...
      logger.info('Redis connection established for threat intelligence database');
    } catch (error) {
  ...
  private async createIndexes(): Promise<void> {
    try {
      ...
      await threatsCollection.createIndex({ intelligenceId: 1 }, { unique: true });
      await threatsCollection.createIndex({ globalThreatId: 1 }, { unique: true });
      ...
      await threatsCollection.createIndex({ 'indicators.indicatorType': 1, 'indicators.value': 1 });

      // Correlation data indexes
      const correlationsCollection = this.db.collection('correlation_data');
      await correlationsCollection.createIndex({ correlationId: 1 }, { unique: true });
      ...
      await validationCollection.createIndex({ validationId: 1 }, { unique: true });
  ...
  private async initializeTAXIICollections(): Promise<void> {
    try {
      ...
      for (const collection of defaultCollections) {
        await taxiiCollection.replaceOne(
          { id: collection.id },
          collection,
          { upsert: true }
        );
  ...
  async storeThreatIntelligence(threat: GlobalThreatIntelligence): Promise<void> {
    try {
      logger.info('Storing threat intelligence', { threatId: threat.threatId, intelligenceId: threat.intelligenceId });
      ...
      this.emit('threat_stored', { threatId: threat.threatId, intelligenceId: threat.intelligenceId });
  ...
  private async storeThreatIndicators(threat: GlobalThreatIntelligence): Promise<void> {
    try {
  ...
  private async updateSTIXObjects(threat: GlobalThreatIntelligence): Promise<void> {
    try {
  ...
  private convertToSTIXFormat(threat: GlobalThreatIntelligence): STIXObject[] {
    ...
    for (const indicator of threat.indicators) {
      const indicatorObject: STIXObject = {
        ...
        valid_until: indicator.expirationDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days
  ...
  private async cacheThreatIntelligence(threat: GlobalThreatIntelligence): Promise<void> {
    try {
      ...
      await this.redis.setex(intelligenceCacheKey, 3600, JSON.stringify(cacheData));
  ...
  async updateThreatIntelligence(threat: GlobalThreatIntelligence): Promise<void> {
    try {
      logger.info('Updating threat intelligence', { threatId: threat.threatId, intelligenceId: threat.intelligenceId });
      ...
      if (result.matchedCount === 0) {
        throw new Error(`Threat intelligence not found: ${threat.intelligenceId}`);
      }
      ...
      this.emit('threat_updated', { threatId: threat.threatId, intelligenceId: threat.intelligenceId });
  ...
  async getThreatById(threatId: string): Promise<GlobalThreatIntelligence | null> {
    try {
      // Check cache first
  ...
  async getThreatByIntelligenceId(intelligenceId: string): Promise<GlobalThreatIntelligence | null> {
    try {
      // Check cache first
      ...
      logger.error('Failed to get threat by intelligence ID:', { error, intelligenceId });
      throw error;
    }
  }

  async getThreatByIndicator(indicatorType: string, indicatorValue: string): Promise<GlobalThreatIntelligence | null> {
    try {
      ...
      logger.error('Failed to get threat by indicator:', { error, indicatorType, indicatorValue });
      throw error;
    }
  }
  ...
  async getRecentThreats(region?: string, limit: number = 10): Promise<GlobalThreatIntelligence[]> {
    try {
      const threatsCollection = this.db.collection('global_threat_intelligence');
  ...
  async getCorrelationCount(region?: string): Promise<number> {
    try {
      ...
      if (region) {
        ...
        const threatsInRegion = await this.db.collection('global_threat_intelligence')
          .find({ regions: region })
          .project({ threatId: 1 })
          .toArray();
  ...
  async storeCorrelationData(correlation: CorrelationData): Promise<void> {
    try {
      logger.info('Storing correlation data', { correlationId: correlation.correlationId });
      
      const correlationsCollection = this.db.collection('correlation_data');
      await correlationsCollection.insertOne(correlation);
      
      this.emit('correlation_stored', { correlationId: correlation.correlationId });
  ...
  async searchThreats(query: any, pagination: PaginationParams): Promise<ApiResponse<GlobalThreatIntelligence[]>> {
    try {
      const threatsCollection = this.db.collection('global_threat_intelligence');
      ...
      const threats = await threatsCollection
        .find(searchQuery)
        .sort({ [pagination.sortBy || 'firstSeen']: pagination.sortOrder === 'desc' ? -1 : 1 })
  ...
  private buildSearchQuery(query: any): any {
    ...
    if (query.confidence && (query.confidence.min !== undefined || query.confidence.max !== undefined)) {
    ...
    if (query.indicators && (query.indicators.types || query.indicators.values)) {
      searchQuery.$or = [];
  ...
  private async getDatabaseStats(): Promise<DatabaseStats> {
    try {
      const [
        ...
        this.db.collection('global_threat_intelligence').estimatedDocumentCount(),
  ...
  private async performDatabaseMaintenance(): Promise<void> {
    try {
      ...
      if (oldCorrelationsResult.deletedCount > 0) {
        logger.info(`Cleaned up ${oldCorrelationsResult.deletedCount} old correlations`);
      }
