...
get_or_create_vcn() {
    ...
    EXISTING_VCN=$(oci network vcn list \
        ...
        --all \
        --output json 2>/dev/null | jq -r '.data[] | select(.display-name == "'"$vcn_name"'") | .id' 2>/dev/null | head -1)
...
get_or_create_subnet() {
    ...
    SUBNET_ID=$(oci network subnet list \
        --compartment-id "$TENANCY_ID" \
        --vcn-id "$vcn_id" \
        --output json 2>/dev/null | jq -r '.data[0].id // empty' 2>/dev/null || echo "")
    ...
    EXISTING_SUBNETS=$(oci network subnet list \
        --compartment-id "$TENANCY_ID" \
        --vcn-id "$vcn_id" \
        --output json 2>/dev/null | jq -r '.data[].cidr-block' 2>/dev/null || echo "")
    
    # Find available CIDR block
    SUBNET_CIDR=""
    for i in {1..10}; do
        TEST_CIDR="10.0.$i.0/24"
        if ! echo "$EXISTING_SUBNETS" | grep -q "$TEST_CIDR"; then
    ...
    SUBNET_RESULT=$(oci network subnet create \
        ...
        --display-name "a1-flex-subnet-${region}-${ad##*:}" \
        --output json 2>&1)
