#!/usr/bin/env bash
# Shared helpers for Stage 1 training jobs.
# Adapted from persona-regeneration-helpers.sh
...
# Lightning.ai specific helpers
check_lightning_cli() {
    if ! command -v lightning >/dev/null; then
        log_error "Lightning CLI not found. Install with: pip install lightning"
        return 1
    fi
    return 0
}

get_lightning_cluster_status() {
    local cluster_id="$1"
    lightning cluster status "${cluster_id}" 2>/dev/null || echo "unknown"
}
