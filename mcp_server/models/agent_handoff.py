...
class HandoffReportMetrics(BaseModel):
    ...
    average_task_duration_seconds: Optional[float] = Field(None, ge=0, description="Average task duration")
    total_processing_time_seconds: Optional[float] = Field(None, ge=0, description="Total processing time")
    memory_usage_peak_mb: Optional[float] = Field(None, ge=0, description="Peak memory usage in MB")
...
class Phase6ComponentProgress(BaseModel):
    ...
    assigned_to: Optional[str] = Field(None, description="Agent ID assigned to this component")
...
class SignOffRecord(BaseModel):
    ...
    comments: Optional[str] = Field(None, max_length=1000, description="Sign-off comments")
    conditions: Optional[List[str]] = Field(None, description="Approval conditions if applicable")
    expires_at: Optional[datetime] = Field(None, description="Sign-off expiration time")
...
class AgentHandoffReport(BaseModel):
    ...
    security_scan_results: Optional[Dict[str, Any]] = Field(None, description="Security scan results")
    performance_benchmarks: Optional[Dict[str, Any]] = Field(None, description="Performance benchmark results")
    ...
    # Review and feedback
    review_feedback: Optional[str] = Field(None, max_length=2000, description="Review feedback")
    ...
    submitted_at: Optional[datetime] = Field(None, description="Report submission timestamp")
    reviewed_at: Optional[datetime] = Field(None, description="Report review timestamp")
    approved_at: Optional[datetime] = Field(None, description="Report approval timestamp")
    ...
    def get_component_progress(self, component_type: ComponentType) -> Optional[Phase6ComponentProgress]:
...
class AgentHandoffSubmissionRequest(BaseModel):
    ...
    attachments: Optional[List[str]] = Field(None, description="Additional attachment paths")
...
class AgentHandoffReviewRequest(BaseModel):
    ...
    amendments_required: Optional[List[str]] = Field(None, description="Specific amendments required")
    quality_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Quality score assigned by reviewer")
...
class AgentHandoffSignOffRequest(BaseModel):
    ...
    comments: Optional[str] = Field(None, max_length=1000, description="Sign-off comments")
    conditions: Optional[List[str]] = Field(None, description="Approval conditions if applicable")
    expires_in_days: Optional[int] = Field(None, ge=1, le=365, description="Sign-off expiration in days")
...
class AgentHandoffFilter(BaseModel):
    ...
    status: Optional[HandoffReportStatus] = Field(None, description="Filter by report status")
    report_type: Optional[str] = Field(None, description="Filter by report type")
    created_after: Optional[datetime] = Field(None, description="Filter reports created after this time")
    created_before: Optional[datetime] = Field(None, description="Filter reports created before this time")
    min_quality_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Minimum quality score")
    sign_off_status: Optional[str] = Field(None, description="Filter by sign-off status")
...
class AgentHandoffStats(BaseModel):
    ...
    average_phase6_progress: Optional[float] = Field(None, ge=0.0, le=100.0, description="Average Phase 6 progress")
    
    # Quality metrics
    average_quality_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Average quality score")
    average_test_coverage: Optional[float] = Field(None, ge=0.0, le=100.0, description="Average test coverage")
    
    # Timing statistics
    average_review_time_days: Optional[float] = Field(None, ge=0.0, description="Average review time in days")
    average_approval_time_days: Optional[float] = Field(None, ge=0.0, description="Average approval time in days")
