...
export class AIStrategyReviewService {
  ...
  static async reviewDocument(documentId: string): Promise<StrategyReview> {
    ...
    if (
      ...
      content.includes('differentiator')
    ) {
    ...
    if (content.includes('market size') && content.includes('growth')) {
      ...
      review.logicalGaps.push(
        'Missing quantification of total addressable market (TAM)',
      )
    }
  ...
  static async performCrossReview(category: string): Promise<{
    ...
    return {
      ...
      contradictions:
        documents.length === 0 ? [] : ['None detected across current set'],
      missingLinks:
        documents.length < 3 ? ['Insufficient inter-document referencing'] : [],
    }
  }
}
