...
router.post('/', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!title || !value) {
        throw new ValidationError('title and value are required', { title: !title ? 'Title is required' : '', value: !value ? 'Value is required' : '' })
    }
...
router.post('/:opportunityId/stage', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!stage) {
        throw new ValidationError('stage is required', { stage: 'Stage is required' })
    }
...
router.post('/:opportunityId/contacts', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!contact) {
        throw new ValidationError('contact is required', { contact: 'Contact is required' })
    }
...
router.post('/:opportunityId/notes', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!note) {
        throw new ValidationError('note is required', { note: 'Note is required' })
    }
