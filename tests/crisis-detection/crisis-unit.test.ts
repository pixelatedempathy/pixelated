...
class MockCrisisDetector {
  ...
  private evaluatePsychosis(content: string): number {
    ...
    if (acuteMatches >= 2) {
      ...
      score = 0.7 // Medium severity - single symptom but worth investigating
    }
...
describe('Phase 4.3: Crisis Intervention Scenario Tests', () => {
  ...
  describe('Test Dataset Validation', () => {
    ...
    it('should have all 5 crisis types represented', () => {
      expect(
        CRISIS_TEST_DATASET_STATS.byType.suicidal_ideation,
      ).toBeGreaterThan(0)
      ...
      expect(CRISIS_TEST_DATASET_STATS.byType.substance_abuse).toBeGreaterThan(
        0,
      )
      expect(
        CRISIS_TEST_DATASET_STATS.byType.psychotic_symptoms,
      ).toBeGreaterThan(0)
    })
  ...
  describe('Escalation Routing', () => {
    ...
    it('should route high and critical cases to escalation or emergency', async () => {
      ...
      for (const testCase of highCases) {
        ...
        if (
          result.type !== 'none' &&
          ['escalation', 'emergency'].includes(result.escalation)
        ) {
          correctCount++
        }
      }
    ...
    it('should route medium and above cases to warning or higher', async () => {
      ...
      for (const testCase of mediumCases) {
        ...
        if (
          result.type !== 'none' &&
          ['warning', 'escalation', 'emergency'].includes(result.escalation)
        ) {
          correctCount++
        }
      }
  ...
  describe('Audit Logging & Compliance', () => {
    it('should flag all crisis detections for audit trail', async () => {
      ...
      auditEvents.forEach((event) => {
        ...
        expect(['emergency', 'escalation', 'warning']).toContain(
          event.escalation,
        )
      })
    })
  ...
  describe('Performance Requirements', () => {
    ...
    it('should handle batch analysis efficiently', async () => {
      ...
      expect(endTime - startTime).toBeLessThan(
        ALL_CRISIS_TEST_CASES.length * 100,
      ) // Total <total_cases * 100ms
    })
  })
})
