...
interface MongoAuthServiceShape {
  verifyAuthToken?: (
    token: string,
  ) => Promise<{ userId: string; role: string; session?: string }>
  ...
  refreshSession?: (
    token: string,
  ) => Promise<{ user: MongoUser; session: unknown; accessToken: string }>
  updateUser?: (
    id: string,
...
function ensureService(): MongoAuthServiceShape {
  if (!mongoAuthService)
    throw new Error('mongoAuthService is not available at runtime')
