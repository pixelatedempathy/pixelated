...
} from './src/lib/auth/working-pg-auth-final'

async function demonstrateWorkingAuthIntegration() {
  try {
    console.log('🚀 Starting Working Better-Auth PostgreSQL Integration Demo')
    ...
    const registerResult = await registerWithBetterAuth(
      {
        email: 'workingdemo.user@example.com',
        password: 'SecurePassword123!',
        name: 'Working Demo User',
        role: 'therapist'
      },
      {
        ip: '127.0.0.1',
