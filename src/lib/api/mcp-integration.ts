...
export class MCPIntegration {
  ...
  async initialize(): Promise<void> {
    try {
      logger.info('Initializing MCP integration', { modelId: this.config.modelId });
  ...
  async processConversation(messages: Array<{ role: string; content: string }>): Promise<MCPResponse> {
  ...
  async analyzeContext(content: string, contextType: string = 'therapeutic'): Promise<MCPResponse> {
    return this.sendRequest({
...
export async function POST(context: APIContext) {
  try {
    ...
    if (!mcp) {
      return new Response(JSON.stringify({ error: 'MCP integration not available' }), {
        status: 500,
