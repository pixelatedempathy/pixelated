...
def main():
    ...
    for idx, file_info in enumerate(all_files, 1):
        ...
        try:
            ...
            try:
                loader.s3_client.head_object(Bucket=bucket, Key=output_key)
                print(f"   ⏭️  Skipping {s3_key}, already processed")
                continue
            except Exception:
                pass  # Doesn't exist, proceed with processing

            # Create DataSource
            source = DataSource(
                name=Path(s3_key).stem, path=s3_path, format="jsonl", size_bytes=file_info["size"]
            )
            ...
            if not processed_records:
                print("   ⚠️  No records passed pipeline filtering")
                continue
            ...
            buffer.seek(0)
            loader.s3_client.upload_fileobj(buffer, bucket, output_key)

            total_processed += len(processed_records)
            print(
                f"   ✅ Processed: {len(processed_records):,} records → s3://{bucket}/{output_key}"
            )
