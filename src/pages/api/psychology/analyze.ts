...
const CLINICAL_PATTERNS = {
  affect: [
    {
      pattern: /depressed|sad|down|hopeless|worthless/gi,
      category: 'depressive',
      severity: 'moderate',
    },
    {
      pattern: /anxious|worried|nervous|panic|afraid/gi,
      category: 'anxiety',
      severity: 'moderate',
    },
    {
      pattern: /angry|mad|furious|rage|irritated/gi,
      category: 'anger',
      severity: 'moderate',
    },
    {
      pattern: /numb|empty|disconnected|void/gi,
      category: 'dissociation',
      severity: 'high',
    },
    {
      pattern: /happy|good|better|positive|hopeful/gi,
      category: 'positive',
      severity: 'low',
    },
  ],
  cognition: [
    {
      pattern: /can't think|confused|foggy|scattered/gi,
      category: 'concentration',
      severity: 'moderate',
    },
    {
      pattern: /racing thoughts|can't stop thinking|overwhelmed/gi,
      category: 'rumination',
      severity: 'moderate',
    },
    {
      pattern: /worthless|failure|stupid|useless/gi,
      category: 'negative_self',
      severity: 'high',
    },
    {
      pattern: /catastrophe|worst case|disaster|terrible/gi,
  ...
  behavior: [
    {
      pattern: /sleep|insomnia|tired|exhausted/gi,
      category: 'sleep',
      severity: 'moderate',
    },
    {
      pattern: /eating|appetite|weight|food/gi,
      category: 'appetite',
      severity: 'moderate',
    },
    {
      pattern: /isolating|alone|avoiding|withdrawn/gi,
      category: 'withdrawal',
      severity: 'moderate',
    },
    {
      pattern: /active|exercising|engaged|motivated/gi,
      category: 'activation',
      severity: 'low',
    },
  ],
  risk: [
    {
      pattern: /suicide|kill myself|end it all|not worth living/gi,
      category: 'suicidal',
      severity: 'high',
    },
    {
      pattern: /cutting|self-harm|hurt myself|punish myself/gi,
      category: 'self_harm',
      severity: 'high',
    },
    {
      pattern: /drinking|drugs|using|high|drunk/gi,
      category: 'substance',
      severity: 'moderate',
    },
    {
      pattern: /hearing voices|seeing things|paranoid|conspiracy/gi,
      category: 'psychosis',
      severity: 'high',
    },
  ],
...
function assessRisk(
  ...
  const suicidalPatterns = {
    passive: /want to die|suicide|kill myself|end my life|not worth it/gi,
    active: /kill myself|end it all|suicide|die/gi,
    plan: /plan to|how to|method|when I|specific way|prepared/gi,
    intent: /going to|will do|tonight|tomorrow|soon|doing it/gi,
  }
  ...
  Object.entries(suicidalPatterns).forEach(([level, pattern]) => {
    ...
    if (matches) {
      ...
      if (level === 'intent') {
        ...
        level === 'active' &&
        !['with_intent', 'with_plan'].includes(suicidalSeverity)
      ) {
  ...
  if (
    (suicidalSeverity as string) === 'with_intent' ||
    highSeverityObs.length > 2
  ) {
    overallRisk = 'imminent'
  } else if (
    (suicidalSeverity as string) === 'with_plan' ||
    riskObservations.length > 1
  ) {
    overallRisk = 'high'
  ...
  return {
    ...
    selfHarm: {
      risk: observations.some((obs) => obs.observation.includes('self-harm'))
        ? 'high'
        : 'low',
      indicators: observations
        .filter((obs) => obs.observation.includes('self-harm'))
        .map((obs) => obs.observation),
    },
    substanceUse: {
      risk: observations.some((obs) => obs.observation.includes('substance'))
        ? 'moderate'
        : 'low',
      indicators: observations
        .filter((obs) => obs.observation.includes('substance'))
        .map((obs) => obs.observation),
    },
    psychosis: {
      risk: observations.some((obs) => obs.observation.includes('psychosis'))
        ? 'high'
        : 'low',
      indicators: observations
        .filter((obs) => obs.observation.includes('psychosis'))
        .map((obs) => obs.observation),
    },
    immediateActions:
      overallRisk === 'imminent'
...
function calculateProgressMetrics(
  ...
  const symptomSeverity = Math.max(
    1,
    Math.min(10, 5 + negativeIndicators - positiveIndicators),
  )

  return {
    symptomSeverity: {
      current: symptomSeverity,
      baseline: clientContext?.history?.previousSessions
        ? symptomSeverity + 1
        : undefined,
      trend:
      ...
      indicators:
        positiveIndicators > negativeIndicators
          ? ['Positive language use', 'Hope expression']
          : ['Distress indicators', 'Negative language'],
    },
    functionalImpairment: {
      current: symptomSeverity,
      baseline: clientContext?.history?.previousSessions
    ...
    therapeuticAlliance: {
      strength:
        content.includes('comfortable') || content.includes('helpful')
          ? 'strong'
          : 'moderate',
      indicators: ['Engaged in session', 'Open communication'],
    },
    goalProgress:
      clientContext?.history?.therapeuticGoals?.map((goal) => ({
        goal,
  ...
  try {
    ...
    if (
      ![
        ...
        'risk',
        'comprehensive',
      ].includes(body.analysisType)
    ) {
      return new Response(
        JSON.stringify({
          error:
            'Invalid analysis type: must be session, progress, intervention, risk, or comprehensive',
        }),
        {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
    ...
    const riskAssessment = options.includeRiskAssessment
      ? assessRisk(body.content, observations)
      : undefined
    const progressMetrics = options.includeProgressMetrics
      ? calculateProgressMetrics(body.content, body.clientContext)
      : undefined
