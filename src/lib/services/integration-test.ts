...
export class ServiceIntegrationTester {
  ...
  private async testMemoryService(): Promise<boolean> {
    try {
      ...
      // Test create
      const memory = await this.memoryService.createMemory('Test memory content', {
        userId,
        tags: ['test'],
      ...
      // Test search
      const searchResults = await this.memoryService.searchMemories(userId, 'Updated');
  ...
  private async testBiasDetection(): Promise<boolean> {
    try {
      ...
      return result.biasScore !== undefined && 
  ...
  private async testCrossServiceCommunication(): Promise<boolean> {
    try {
      ...
      const sessionText = 'I have been feeling overwhelmed lately with work stress';
      ...
      const encryptedAnalysis = await fheService.encrypt(JSON.stringify(emotionResult));
