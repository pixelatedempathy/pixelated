...
test.describe('Journal Research Report Generation Workflow', () => {
  ...
  test('displays report list', async ({ page }) => {
    ...
    const reportList = page.locator(
      'table tbody tr, [data-testid="report-item"]',
    )
  ...
  test('generates session report', async ({ page }) => {
    ...
    const generateButton = page.locator(
      'button:has-text("Generate Report"), button:has-text("Create Report")',
    )
    ...
    if ((await generateButton.count()) > 0) {
      ...
      await page.waitForSelector('form, select[name="reportType"]', {
        timeout: 5000,
      })
      ...
      await expect(
        page.locator('text=Generating, text=Report, text=Progress'),
  ...
  test('generates weekly report', async ({ page }) => {
    ...
    if ((await generateButton.count()) > 0) {
      ...
      await expect(
        page.locator('text=Generating, text=Weekly Report'),
      ).toBeVisible({ timeout: 5000 })
    }
  })
  ...
  test('views generated report', async ({ page }) => {
    ...
    const firstReportLink = page
      .locator('table a, [data-testid="report-link"]')
      .first()
    ...
    if ((await firstReportLink.count()) > 0) {
      ...
      await expect(page.locator('h1')).toBeVisible()
      await expect(
        page.locator('text=Report, text=Content, text=Summary'),
      ).toBeVisible()
    }
  })
    ...
    if ((await firstReportLink.count()) > 0) {
      ...
      if ((await exportButton.count()) > 0) {
        ...
        if (download) {
          ...
          await expect(
            page.locator('text=Export, text=Download, text=Saved'),
          ).toBeVisible({ timeout: 5000 })
        }
      }
    }
  ...
  test('displays report in different formats', async ({ page }) => {
    ...
    if ((await firstReportLink.count()) > 0) {
      ...
      await expect(
        page.locator('text=Report, pre, [data-testid="report-content"]'),
      ).toBeVisible()
    }
  })
  ...
  test('filters reports by type', async ({ page }) => {
    ...
    const filterSelect = page
      .locator('select, [data-testid="report-type-filter"]')
      .first()
